...
- 제약 부재 시 해당 프로퍼티 접근 X
  - 해당 프로퍼티 부재 타입 가능성 有
- property of the `a` and `b` parameters. Without the type constraint, we wouldn’t be able to access those properties because the values might have been some other type without a length property.
