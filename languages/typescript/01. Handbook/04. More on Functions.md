...
- `Output` <sub>(타입 매개변수)</sub>
  - 함수 표현식 반환값 <sub>(`parseInt(n)`)</sub>
- type parameter (from the given `string` array), as well as the `Output` type parameter based on the return value of the function expression (`number`).
```ts
