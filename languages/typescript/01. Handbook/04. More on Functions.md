...
//                                  this      : globalThis
//    admins: User[]                this.admin: any
const admins = db.filterUsers(() => this.admin);
```

##### `this` 매개변수 · 타 매개변수 함께 사용 가능
- 매개변수들 중 반드시 1번째 매개변수 선언 필수
- 타 매개변수 <sub>(2번째 ~)</sub>
  - 함수 실제 사용 <sub>(호출)</sub> 순서
```ts
interface User {
  name: string;
}

// this 1번째 선언 · 타 매개변수 이어 선언
function printInfo(this: User, age: number, message: string) {
  console.log(`${this.name}, ${age}세: ${message}`);
}

const admin: User = { name: '홍길동' };

// 호출 시 this는 객체 형태의 문맥으로 제공하고, 실제 인수는 두 번째 매개변수부터 전달합니다.
printInfo.call(admin, 25, '반갑습니다.');
...
##### 값 미반환 함수 반환 타입
- 타입 추론
  - 함수 내 `return` <sub>(문)</sub> 無
  - `return` <sub>(문)</sub> 내 명시적 반환값 無
```ts
// void (반환 타입) 추론
function noop() {
  return;
...
##### JS 함수 == 객체형
- 프로퍼티 有
- `Object.prototype` 상속 <sub>(프로토타입 체인)</sub>
