...
// 전달 함수 내 인수 (s) 타입 추론
// - 배열 · 메서드 타입 정보 이용
sArray.forEach(function (s) { // s: string
  s.toUpperCase();
});

// 화살표 함수 동일 적용
sArray.forEach((s) => { // s: string
  s.toUpperCase();
});
```
...
- `prop?: type`
```ts
function f(o: { s1: string; s2: string }) {
  // …
}
...
function f(o: { s1: string; s2: string }) {
  ...
  o.s2.toUpperCase(); // 'o.s2' is possibly 'undefined'.
...
- 이전 단계 타입 확인 불필요
```ts
function welcomePeople(a: string[] | string | number) {
