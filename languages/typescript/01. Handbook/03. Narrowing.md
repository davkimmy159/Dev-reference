...
- `x`
  - `union`
- `true` 분기
  - optional · 필수 프로퍼티 <sub>(`value`)</sub>
- `false` 분기
  - optional · 부재 프로퍼티 <sub>(`value`)</sub>
```ts
type Fish = { swim: () => void };
type Bird = { fly: () => void };
...
##### `parameterName is Type` <sub>(타입 predicate)</sub>
- ex\) `pet is Fish`
- `parameterName`
  - 함수 매개변수
...
##### 식별된 `union`
- `union` 멤버 내 공통 프로퍼티 <sub>(문자열 상수)</sub> 有
- 타입 좁히기 가능
...
##### `union` 내 모든 멤버 제거 시 <sub>(타입 좁히기 후)</sub>
- `never` <sub>(타입)</sub>
  - 부재 상태 표현
