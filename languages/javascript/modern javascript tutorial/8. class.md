...
### `getter` · `setter`
- `getter` · `setter` · 계산된 프로퍼티 (computed property) 지원
```javascript
class User {
...
```
- 상속 클래스 생성자 `this` 사용 전 반드시 `super(…)` 호출 (부모 생성자 실행)
- 생성자 호출 시 자바스크립트 생성자 함수 구분
  - 일반 클래스 생성자 함수
  - 상속 클래스 생성자 함수 (derived constructor)
...
- 팩토리 메서드
  - 다양한 방법 사용해 조건 맞는 `article` 인스턴스 생성
    1. 매개변수 (`title` · `date` 등) 이용해 관련 정보 담긴 `article` 생성
