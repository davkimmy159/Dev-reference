...
alert( bag.apple ); // fruit 에 'apple' 할당 시 5 출력
...
```
  ...
  - `src1, src2, src3...`
    - 복사하고자 하는 객체
  1. 객체 `src1, src2, src3...` 프로퍼티 `dest` 에 복사
  2. `dest` 반환
```javascript
let user = { name: "John" };
...
- "John" 도달 가능한 상태 X
  - 메모리에서 제거
  - "John" 저장된 데이터 <sub>(프로퍼티)</sub> 메모리에서 제거
- 외부로 나가는 참조
  - 도달 가능한 상태 영향 X
- 외부에서 들어오는 참조
  - 도달 가능한 상태 영향
...
// user.address 프로퍼티 미존재 시 2번째 점 연산자 에러 발생
alert( user?.address );        // undefined
alert( user?.address.street ); // undefined
```

<img class="icon" src="./images/commons/icons/triangle-exclamation-solid.svg" /> **옵셔널 체이닝 `?.` 남용 X**

- 존재하지 않아도 괜찮은 대상에만 사용
- 논리상 필수값에 사용 X
```javascript
// 논리상 user 필수값
// 논리상 address 필수값 X → (?.) 사용
user.address?.street;
...
<img class="icon" src="./images/commons/icons/triangle-exclamation-solid.svg" /> **`?.` 앞 변수 반드시 선언 필요**
...
<img class="icon" src="./images/commons/icons/triangle-exclamation-solid.svg" /> **`?.` : 읽기 · 삭제 가능, 쓰기 X**
...
- 심볼형 값 다른 자료형으로 암시적 <sub>(자동)</sub> 형 변환 X
```javascript
let id = Symbol("id");
...
- 문자열 · 심볼
  ...
  - 자바스크립트 '언어 차원의 보호장치' 마련해 심볼형 다른 형으로 변환되지 않게 방지
...
- 객체 형 변환
  - 구분 기준 : 'hint' 값 <sub>('목표로 하는 자료형')</sub>
...
- 드물게 발생
- 이항 덧셈 연산자 `+`
  - 피연산자 자료형 따라 문자열 합치는 연산 · 숫자 더해주는 연산
  - `+` 연산자 인수 객체 시
    - hint : default
- 동등 연산자 `==`
  - 객체 · 문자 · 숫자 · 심볼형 비교 시 객체 어떤 자료형 변환 확신 X
    - hint : default
```javascript
...
3. ① · ② 해당 X, hint : number · default
    - `obj.valueOf()` · `obj.toString()` 호출 <sub>(존재 메서드만 실행)</sub>

#### `Symbol.toPrimitive`
- 목표 자료형 <sub>(hint)</sub> 명명
```javascript
...
- `Symbol.toPrimitive` 무조건 원시 자료 반환
  - 미반환 시 에러 발생
