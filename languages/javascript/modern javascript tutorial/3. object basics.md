...
- 복수 단어 조합 프로퍼티 키
  ...
  user.likes.bi#ds = true; // '$', '_' 제외 특수 문자
  ```
...
### 생성자 함수
- 생성자 함수 (constructor function), 일반 함수 기술적 차이 X
- 두 관례
  - 함수 이름 첫 글자 대문자
  - 반드시 'new' 연산자 붙여 실행
- 생성자 함수 실행 알고리즘
  1. 빈 객체 생성 후 this에 할당
  2. 함수 본문 실행
...
```
- 모든 함수 : 생성자 함수 가능
  - new 붙여 실행 시 모든 함수 생성자 알고리즘 실행
- '첫 글자 대문자’ 함수 : 반드시 new 붙여 실행

#### new function() { … }
- 재사용 필요 없는 복잡한 객체
  - 익명 생성자
    - 익명 함수 (어디에도 저장 X)
    - 단 한 번만 호출 목적 (재사용 불가능)
    - 코드 캡슐화
```javascript
 // 익명 생성자 함수
...
```
- 일반적인 방법 함수 호출 시 'new 함께 호출' 동일 동작 트릭
```javascript
function User(name) {
...
let bora = User("보라");   // 'new User' 쓴 것처럼 바꿈
alert(bora.name);          // 보라
```
- new 함께 호출 or 아니든 코드 동일 동작
  - 좀 더 유연하게 코드 작성
- new 있으면 새로운 객체 생성 누구나 알 수 있음
  - 'new 없이 객체 생성' 남발 X

### 생성자와 return문
- return 문 존재 시
  - 객체 return 　: this 대신 객체 반환
  - 원시형 return : 무시 → this 반환
```javascript
function BigUser() {
  this.name = "원숭이";
  return { name: "고릴라" };    // this 아닌 새로운 객체 반환
}
...
```
- 명세서 옵셔널 체이닝 추가 전 '&&' 연산자 사용해 문제 해결
...
### 옵셔널 체이닝 등장
- '앞’ 평가 대상 undefined · null 이면 평가 중단, undefined 반환
- ‘앞’ 평가 대상에만 동작, 확장 X
```javascript
let user = {};                  // 주소 정보 없는 사용자
...
// user?. → null · undefined 경우만 처리
// 실제 값 존재 시 user.address 프로퍼티 반드시 필요
// user.address 프로퍼티 미존재 시 두 번째 점 연산자 에러 발생
...
// 그렇지 않으면 에러 초기 발견 X → 디버깅 난도 ↑
```

#### '?.' 앞의 변수 반드시 선언되어 있어야 함
```javascript
// 변수 user 미선언 시 평가 때 에러 발생
// 'let', 'const', 'var' 사용해 변수 user 정의 (선언 완료 변수 대상으로만 동작)
...
### ?.() vs ?.[]
- '?.' ≠ 연산자
  - 함수 · 대괄호 함께 동작하는 특별한 문법 구조체 (syntax construct)
- 존재 여부 불확실한 함수 호출
```javascript
let user1 = {
  admin() {
...
- . 대신 대괄호 []
  - ?.prop → ?.[key]
- 동일하게 존재 여부 불확실한 객체의 프로퍼티 안전하게 접근
...
#### 심볼은 문자형 자동 형 변환 X
- 자바스크립트 문자형 암시적 형 변환 비교적 자유롭게 발생
  - alert 함수 거의 모든 값 인자로 받을 수 있음
- 심볼형 값 다른 자료형으로 암시적 형 변환 (자동 형 변환) X
```javascript
let id = Symbol("id");
...
- 문자열 · 심볼 : 근본 다름
  ...
  - 자바스크립트 '언어 차원의 보호장치(language guard)' 마련해 심볼형 다른 형으로 변환되지 않게 방지
- 심볼 출력 시 .toString() 메서드 명시적으로 호출
```javascript
let id = Symbol("id");
...
### ‘숨김’ 프로퍼티
- 심볼 이용 ‘숨김 (hidden)’ 프로퍼티 추가
- 외부 코드에서 접근 불가능, 값 덮어쓸 수 없음
```javascript
let user = {       // 서드파티 코드 객체
  name: "John"
};
...
```
- 서드파티 코드 객체 함부로 새로운 프로퍼티 추가 X
- 서드파티 코드에서 심볼 접근 X
  - 서드파티 코드 모르게 식별자 부여
- ex) 서드파티 코드, 현재 작성 스크립트, 제3 스크립트 각자 서로 코드 모른 채 user 식별해야 하는 상황
  - 제3 스크립트 Symbol("id") 전용 식별자 사용
```javascript
let id = Symbol("id");
user[id] = "제3 스크립트 id 값";
```
- 심볼 유일성 보장
  - 이름 같아도 코드 식별자와 제3 스크립트 식별자 충돌 X
- 문자열 "id" 식별자 시용 시 충돌 발생 가능
```javascript
let user = { name: "John" };
...
// 제3 스크립트 동일한 문자열 "id" 식별자 생성 시
// 의도치 않게 값 덮어 쓰여 식별자 무의미
user.id = "제3 스크립트 id 값"
```

#### Symbols in a literal
- 객체 리터럴 {...} 객체 생성 시 대괄호 사용해 심볼형 키 추가
```javascript
let id = Symbol("id");
let user = {
  name: "John",
  [id]: 123 // "id": 123 X (문자열 X)
};
```

#### 심볼 for…in 에서 배제
- 심볼 키 프로퍼티 for..in 반복문에서 배제
```javascript
let id = Symbol("id");
let user = {
  name: "John",
...
```
- Object.keys(user)에서 심볼 키 프로퍼티 배제
- '심볼형 프로퍼티 숨기기 (hiding symbolic property)' 원칙
  - 외부 스크립트, 라이브러리 심볼형 키 프로퍼티 접근 X
- Object.assign 심볼 키 프로퍼티 포함 객체 내 모든 프로퍼티 복사
  - 객체 복사 · 병합 시 대개 id 같은 심볼 포함 프로퍼티 전부 사용 예상 설계
```javascript
let id = Symbol("id");
let user = {
  [id]: 123
...
- 전역 심볼 레지스트리 내부 심볼 생성 후 해당 심볼 접근 시 이름 같으면 항상 동일 심볼 반환
- Symbol.for(key)
  - 레지스트리 내부 심볼 읽기 · 생성
  - 호출 시 key 이름 심볼 반환
  - 조건 맞는 심볼 없으면 새로운 key 심볼 생성 후 레지스트리 내부 저장
- 어플리케이션 내부 광범위 사용 심볼
```javascript
// 전역 레지스트리 내부 심볼 읽기
// 심볼 미존재 시 새로운 심볼 생성
...
- 자바스크립트 내부 사용 심볼
- 시스템 심볼 활용 객체 미세 조정
- 명세서 내 표 · 잘 알려진 심볼 (well-known symbols)
  - Symbol.hasInstance
...
- 숫자형 형 변환
  - 객체끼리 수학 관련 함수 적용 시
  - ex) Date 객체 : date1 - date2
    - 두 날짜 시간 차이 반환
- 문자형 형 변환
  - 대개 객체 출력 시

### ToPrimitive
- 특수 객체 메서드 사용 시 숫자형 · 문자형 형 변환 원하는 대로 조절 가능
- 객체 형 변환
  - 구분 기준 : 'hint’ 값 (‘목표로 하는 자료형’)
#### "string" (hint : string)
  - 문자열 기대 연산 수행 시
```javascript
// 객체 출력
...
- 드물게 발생
- 이항 덧셈 연산자 '+'
  - 피연산자 자료형 따라 문자열 합치는 연산 or 숫자를 더해주는 연산
  - '+' 연산자 인수 객체 시 hint : default
- 동등 연산자 '=='
  - 객체-문자·숫자·심볼형 비교 시 객체 어떤 자료형 변환 확신 X → hint : default
```javascript
// 이항 덧셈 연산 (hint : `default`)
let total = obj1 + obj2;

// obj == number 연산 (hint : `default`)
if (user == 1) { ... };
```
- 비교 연산자 '<' · '>'
  - 피연산자 문자형 · 숫자형 둘 다 허용
...
#### 객체 형 변환 알고리즘
1. 객체 obj\[Symbol.toPrimitive\](hint) 메서드 검색, 존재 시 메서드 호출
    - Symbol.toPrimitive
      - 시스템 심볼
      - 심볼형 키로 사용
2. 1 해당 X, hint : 'string'
    - obj.toString() · obj.valueOf() 호출 (존재 메서드만 실행)
3. 1 · 2 해당 X, hint : 'number' · 'default'
    - obj.valueOf() · obj.toString() 호출 (존재 메서드만 실행)
...
```
- obj[Symbol.toPrimitive] 메서드 하나로 모든 종류 형 변환 가능
```javascript
let user = {
  name: "John",
  money: 1000,
...
- ‘평범한’ 메서드
- '구식’ 형 변환
- 객체 Symbol.toPrimitive 미존재 시 toString · valueOf 호출
  - hint : 'string’
    - toString → valueOf 순
  - 그 외
    - valueOf → toString 순
...
- 모든 형 변환 한 곳 처리 필요 시
  - toString만 구현
- Symbol.toPrimitive, valueOf 미존재 시 toString 모든 형 변환 처리
```javascript
let user = {
  name: "John",
...
### 반환 타입
- 객체 형 변환 메서드
  - 'hint’ 명시 자료형 형 변환 보장 X
    - toString() 항상 문자열 반환 보장 X
    - Symbol.toPrimitive('number') 항상 숫자형 반환 보장 X
  - 확실 : 객체 아닌 원시값 반환
#### 과거의 잔재
- toString · valueOf 객체 반환 시 에러 X
  - 반환 값 무시, 메서드 자체 존재하지 않았던 것처럼 동작
    - 과거 자바스크립트 '에러’ 개념 정립 부족
- Symbol.toPrimitive 무조건 원시자료 반환
  - 미반환 시 에러 발생

### 추가 형 변환
- 상당수 연산자 · 함수 피연산자 형 변환
  - 객체 피연산자
    1. 객체 원시형 변환
