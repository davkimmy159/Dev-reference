...
<img class="icon" src="./images/commons/icons/triangle-exclamation-solid.svg" /> **`?.` 앞 변수 반드시 선언**
...
- '심볼형 프로퍼티 숨기기' 원칙
  - 외부 스크립트 ·라이브러리
    - 심볼형 키 프로퍼티 접근 X
- `Object.assign`
  - <sub>(심볼 키 프로퍼티 포함)</sub> 객체 내 모든 프로퍼티 복사
  - 객체 복사 · 병합 시
    - 프로퍼티 전부 사용 예상 설계
      - ex&#41; `id` 심볼 포함 등
```javascript
...
- `Symbol.for(key)`
  ...
  - 조건 맞는 심볼 부재
    ...
    2. 레지스트리 내부 저장
    3. 반환
- 어플리케이션 내부 광범위 사용 심볼에 사용
```javascript
// 전역 레지스트리 내부 심볼 읽기
// 심볼 미존재 시 새로운 심볼 생성 · 저장 · 반환
...
#### 객체 형 변환 알고리즘
1. 객체 `obj[Symbol.toPrimitive](hint)` 메서드 존재 시 호출
    - `Symbol.toPrimitive`
      - 시스템 심볼
      - 심볼형 키
2. ① 미해당 · hint : 'string'
...
- '평범한' 메서드
- 구식 형 변환
- 객체 `Symbol.toPrimitive` 미존재 시 호출
  - hint : 'string'
    - `toString` → `valueOf` 순
  - hint : 'numver' · 'default'
    - `valueOf` → `toString` 순
