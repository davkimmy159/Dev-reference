...
<script>
  div.insertAdjacentHTML('before begin', '<p>beforebegin</p>');
  div.insertAdjacentHTML('after begin', '<p>afterbegin</p>');
  div.insertAdjacentHTML('before end', '<p>beforeend</p>');
  div.insertAdjacentHTML('after end', '<p>afterend</p>');
</script>
...
##### 래퍼 역할
- 그룹 <sub>(다수 노드 구성)</sub> 감싸 다른 곳 전달
- 문서 상 다른 노드 추가 가능
- 삽입 시
  - 본체 사라짐
  - 내부 노드만 남음
