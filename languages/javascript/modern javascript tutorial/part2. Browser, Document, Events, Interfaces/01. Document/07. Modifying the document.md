...
##### DOM 조작
- '생동감 있는' 웹페이지 만들기 위한 핵심

### 예제 : 메시지 보여주기
- `alert` 창보다 보기 좋은 메시지 창 출력
- HTML 사용해 메시지 창 생성
```html
<style>
.alert {
...
##### `document.createElement(tag)` <sub>(메서드)</sub>
- 새 요소 노드 생성
  - `tag` <sub>(태그명)</sub> 사용
```javascript
let div = document.createElement('div'); <sub>(메서드)</sub>
```
...
### 삽입 메서드
div가 페이지에 나타나게 하려면 document 내 어딘가에 div를 넣어줘야 합니다. document.body로 참조할 수 있는 `<body>`안 같은 곳에 말이죠.

요소 삽입 메서드 append를 사용한 코드 document.body.append(div)를 사용해 직접 새롭게 만든 요소 노드를 페이지에 나타나도록 해봅시다.
...
![](../../images/02/01/07/before-prepend-append-after.svg)
...
예시를 살펴봅시다. 아래 함수 getListContent는 `<li>` 노드로 구성된 fragment를 만들어 줍니다. 이 fragment를 `<ul>`에 추가해 보겠습니다.
```html
<ul id="ul"></ul>
...
먼저 `<th>`를 사용해 표의 헤더(header)를 만든 후 요일을 적어줍니다.
d = new Date(year, month-1) 형식으로 날짜 객체를 생성합니다. 이러면 month 로 대입한 월의 첫 번째 날을 구할 수 있습니다. (자바스크립트에서는 월을 0부터 시작한다는 점에 유의하세요.)
첫번째 열부터 d.getDay() 로 구한 각 달의 첫번째 날까지의 빈칸은 `<td></td>` 로 채워주세요.
d.setDate(d.getDate()+1)을 사용해 d객체의 날짜를 증가시키세요. d.getMonth() 로 구한 달이 다음 달로 변하기 전까지 `<td>`를 사용해 새로운 칸을 추가하면 됩니다. 만약 새로운 칸이 일요일이라면 `“</tr><tr>”` 를 사용해 다음 주로 넘어가세요.
달력이 완성되었지만 마지막 행에 빈 공간이 남아있다면 빈 `<td>` 를 사용해 달력을 사각형으로 만들 수 있습니다.
