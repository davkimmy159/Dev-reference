...
##### `<html>` · `<body>` <sub>(태그)</sub> 부재 시 자동 삽입 <sub>(적절한 위치)</sub>
- 적절한 위치 내 자동 삽입
- ex\) 문서 내 텍스트 <sub>("안녕하세요.")</sub> 하나만 삽입 시
  - 태그 자동 생성 <sub>(브라우저)</sub>
    - `<html>` <sub>(최상위 태그)</sub>
    - `<head>` <sub>(태그)</sub>
    - `<body>` <sub>(태그)</sub>
...
##### [노드 타입](https://dom.spec.whatwg.org/#node) <sub>(총 12가지 · 실무 주 사용 : 4 가지)</sub>
...
##### 다수 하위 패널 <sub>(Elements 패널 우측)</sub>
- Style
