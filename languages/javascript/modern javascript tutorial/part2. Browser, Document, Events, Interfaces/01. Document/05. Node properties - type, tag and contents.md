...
### `nodeName` · `tagName` <sub>(이름 프로퍼티)</sub>
```javascript
...
### `nodeValue` · `data` <sub>(텍스트 노드 내용 조작 프로퍼티)</sub>
...
##### `1` 출력 <sub>(마지막 DOM 노드 == `<script>` 자신)</sub>
- `<script>` 실행 시점
