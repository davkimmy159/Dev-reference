...
- 더 괜찮은 범용 메서드 <sub>(브라우저별 예외 無)</sub>
...
##### [`scrollBy(x, y)`](https://developer.mozilla.org/ko/docs/Web/API/Window/scrollBy) <sub>(메서드)</sub>
- 상대 기준 조정
  - 현재 위치 기준
- ex\) `scrollBy(0,10)`
  - 현재 기준 `10px` 하단 이동
...
##### [`scrollTo(pageX, pageY)`](https://developer.mozilla.org/ko/docs/Web/API/Window/scrollTo) <sub>(메서드)</sub>
- 절대 좌표 기준 조정
