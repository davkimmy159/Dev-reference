...
##### `<tbody>` · `<thead>` · `<tfoot>` <sub>(요소)</sub>
- `rows`
  - `<tr>` <sub>(요소)</sub> 컬렉션

#### `<tr>` <sub>(요소)</sub>
- `cells`
  - `<td>` <sub>(요소)</sub> · `<th>` <sub>(요소)</sub> 컬렉션
- `sectionRowIndex`
  - 는 주어진 `<tr>`이 `<thead>`/`<tbody>`/`<tfoot>`안쪽에서 몇 번째 줄에 위치하는지를 나타내는 인덱스를 반환합니다.
- `rowIndex`
  - `<tr>` <sub>(요소)</sub> 순서 <sub>(`<table>` 요소 내)</sub>
