...
##### `children`
- 자식 노드 중 요소 <sub>(노드)</sub>

##### `[first·last]ElementChild`
- 1번째 마지막 자식 요소 <sub>(노드)</sub>
...
// document (부모 노드)
alert( documentElement.parentNode );

// null (부모 요소 노드)
alert( documentElement.parentElement );
```

##### 사소한 차이 <sub>(`documentElement` 상이 부모 노드 반환)</sub> 활용법
- `<html>` <sub>(요소)</sub> 까지 거슬러 올라가기
...
##### `document.body` 자식 요소 <sub>(노드)</sub> 출력
- `childNodes` → `children`
  - 자식 노드 중 요소 <sub>(노드)</sub> 만 출력
```html
<!DOCTYPE HTML>
<html>
...
##### 2. No
- `elem.children[0]`
  - '요소 <sub>(노드)</sub>' 중 1번째 자식 노드
  - 앞쪽 다른 노드 <sub>(요소 노드 X)</sub> 존재 가능
    - ex\) 텍스트 노드 등
