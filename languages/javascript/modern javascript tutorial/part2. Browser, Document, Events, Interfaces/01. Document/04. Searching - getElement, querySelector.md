...
##### `querySelector` VS `querySelectorAll(css)[0]`
- 차이점
  - 선택자 대응 모든 요소 검색 후 1번째 요소 반환
    - 느림
하고, elem.querySelector는 해당하는 요소를 찾으면 검색을 멈춘다는 점에서 차이가 있습니다. elem.querySelector가 더 빠른 이유이죠. querySelector는 querySelectorAll에 비해 코드의 길이가 짧다는 장점도 있습니다.
