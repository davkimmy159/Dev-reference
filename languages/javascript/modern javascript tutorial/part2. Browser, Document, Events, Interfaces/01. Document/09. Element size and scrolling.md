...
##### 일부 코드 <sub>(`getComputedStyle(elem).width`)</sub>
- 스크롤바 존재 시
  - 비정상 동작

##### 브라우저 간 차이
- 이런 일은 스크롤바가 콘텐츠 영역을 차지하는 몇몇 브라우저에서 발생합니다. 이들 브라우저에선 콘텐츠가 실제 차지하는 영역이 CSS로 설정한 너비보다 좁은데, `clientWidth`와 `clientHeight`는 이를 고려해 클라이언트 요소가 차지하는 공간을 계산합니다.
