...
##### 조상 요소 `position` <sub>(CSS 프로퍼티)</sub> 유무 여부
...
##### `scrollWidth == 324`
- 테두리 <sub>(`border` CSS 프로퍼티)</sub> 내 영역 전체 너비
  - `324px` <sub>(`clientWidth`)</sub>

##### `scrollHeight == 723`
- 테두리 <sub>(`border` CSS 프로퍼티)</sub> 내 영역 전체 높이
...
##### 좌표 시작점
- 테두리 내 영역 <sub>(`client[Width·Height]`)</sub> 좌상단
