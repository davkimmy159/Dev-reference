...
##### `clientWidth` : `324px`
- `284px` <sub>(컨텐츠 너비)</sub> + `2 * 20px` <sub>(좌 · 우 패딩)</sub>
- 콘텐츠 너비 : `284px`
...
- `scrollHeight` 내 `scrollTop` 차지 영역
...
- 요소 너비 · 높이 계산 방법 '지정'
- 값 변경 시
  - `getComputedStyle` <sub>(함수)</sub> 값 부정확
...
##### 공 요소 `position`<sub>(CSS 프로퍼티)</sub>
- `position: absolute`

##### `left`· `top` 측정 기준
- 최근접 조상 요소 `position` <sub>(CSS 프로퍼티)</sub> 가짐
- coordinates are measured from the nearest positioned element, that is `#field` (because it has `position:relative`).
