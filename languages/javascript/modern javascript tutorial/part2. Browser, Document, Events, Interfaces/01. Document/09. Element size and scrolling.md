...
##### `clientWidth` : `284px`
- `300px` <sub>(너비)</sub> - `16px`<sub>(세로 스크롤바 너비)</sub>
- 주의사항
  - 세로 스크롤바 너비 <sub>(`16px`)</sub>
px   차지하는 너비 `16px` 때문에 콘텐츠 너비는 `300px`이 아닌 `284px`이 된다는 점입니다. 따라서 `clientWidth`는 콘텐츠 너비 `284px`에 왼쪽, 오른쪽 패딩(`2 * 20px`)을 더한 값인 `324px`가 됩니다.
