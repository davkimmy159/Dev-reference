...
#### 테두리 <sub>(요소 내 위치 · `border` CSS 프로퍼티)</sub>

##### `client[Top·Left]` 이용해 두께 측정 가능 <sub>(샘플 예시)</sub>
- 좌측 테두리 너비 == `clientLeft` <sub>(`25`)</sub>
- 상단 테두리 높이 == `clientTop` <sub>(`25`)</sub>

![metric-client-left-top](../../images/02/01/09/metric-client-left-top.svg)

##### 정확히는 테두리 내 상대 좌표 <sub>(테두리 외부 기준)</sub>
- 테두리 높이 · 너비 정확히 일치 X
...
##### 패딩 부재 시
- 두 영역 정확히 일치 <sub>(스크롤바 너비 제외)</sub>
  - `client[Width·Height]`
  - 콘텐츠 너비 · 높이
