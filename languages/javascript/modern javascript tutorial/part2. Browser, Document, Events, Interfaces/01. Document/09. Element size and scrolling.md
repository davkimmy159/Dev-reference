...
##### 촤근접 조상 요소 `position` <sub>(CSS 프로퍼티)</sub> 유무 여부
- 존재 시
  - `absolute`
  - `relative`
  - `fixed`
...
##### `clientHeight` : `240px`
- `200px` <sub>(높이)</sub> + `2 * 20px`<sub>(상 · 하 패딩)</sub>
- 테두리 <sub>(`border`)</sub> 안 높이 총합
  - 가로 스크롤바 X

##### `clientWidth` : `324px`
- `284px` <sub>(콘텐츠 너비)</sub> + `2 * 20px` <sub>(좌 · 우 패딩)</sub>
- 콘텐츠 너비 : `284px`
  - `300px` <sub>(너비)</sub> - `16px`<sub>(스크롤바 너비)</sub>

##### 패딩 부재 시 `client[Width·Height]`
- 콘텐츠 영역 크기 <sub>(너비 · 높이)</sub>
  - 정확히 일치
...
##### `scrollWidth` : `324px`
- 콘텐츠 영역 안쪽 전체 너비
  - 가로 스크롤바 의래 가려진 영역 포함
- 가로 스크롤바 X
  - `clientWidth` 동일

##### `scrollHeight` : `723px`
- 콘텐츠 영역 안쪽 전체 높이
  - 세로 스크롤바 의해 가려진 부분 포함
...
<img class="icon" src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`scroll[Left·Top]` : 수정 가능**
...
##### Windows 이외 OS 설치 브라우저 <sub>(Firefox 등)</sub>
- 스크롤바 영역 무시
...
##### 대다수 프로퍼티 <sub>(`scroll[Left·Top]`)</sub>
- 읽기 전용
...
### `scrollTop` 반대편 높이 구하기
- `scrollBottom` <sub>(변수)</sub>
- 임의 요소 <sub>(`elem`)</sub> 기준
...
```javascript
// 스크롤 의해 가려진 바닥 높이 = 최대 높이 - 스크롤 의해 가려진 상단 높이 - 보이는 요소 높이
...
- 최근접 조상 요소
  - 특정 `position` <sub>(CSS 프로퍼티)</sub> 값 가짐
- `#field` <sub>(요소)</sub>
  - `position: relative`
...
##### ① 타입
- `getComputedStyle(elem).width` <sub>(함수)</sub>
...
- `clientWidth`
  - 콘텐츠 · 패딩 영역 포함
##### ④ 스크롤바 존재 시 포함 여부 <sub>(브라우저가 따로 처리)</sub>
- `getComputedStyle(elem).width` <sub>(함수)</sub>
  - 브라우저 따라 상이
- `clientWidth`
