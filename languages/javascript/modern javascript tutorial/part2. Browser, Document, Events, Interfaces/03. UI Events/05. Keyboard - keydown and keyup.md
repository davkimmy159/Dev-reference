...
Or an alternative approach would be to track the input event – it triggers after any modification. There we can check the new value and highlight/modify it when it’s invalid.

##### 개선점
- 마우스 우클릭 + 붙여넣기 방지
- 기타 등등

##### `input` <sub>(이벤트)</sub> 추적 <sub>(대안)</sub>
- 필드 입력값 수정 시 발생
- 핸들러 내 다양한 동작 수행
  - 새 값 확인 · 강조 · 수정 등
