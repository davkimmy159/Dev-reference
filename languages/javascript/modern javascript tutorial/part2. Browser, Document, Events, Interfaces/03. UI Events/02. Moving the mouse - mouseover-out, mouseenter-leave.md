...
|---|---|
|`#parent` → `#child`|`#parent` `mouseout` <sub>(부모 이탈)</sub>|
||`#child` `mouseover` <sub>(자식 진입 · 버블링)</sub>|
|`#parent` ← `#child`|`#parent` `mouseover` <sub>(부모 진입)</sub>|
||`#child` `mouseout` <sub>(자식 이탈 · 버블링)</sub>|

|마우스 이동|발생 이벤트|
|---|---|
|`#parent` → `#child`|`#parent` 이탈|
||`#child` 진입 <sub>(버블링)</sub>|
|`#parent` ← `#child`|`#parent` 진입|
||`#child` 이탈 <sub>(버블링)</sub>|

##### `#parent` <sub>(요소)</sub> → `#child` <sub>(요소)</sub>
1. `mouseout [target: parent]`
    - `#parent` <sub>(요소)</sub> 이탈
