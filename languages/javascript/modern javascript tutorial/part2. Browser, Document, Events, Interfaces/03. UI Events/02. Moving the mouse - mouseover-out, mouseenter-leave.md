...
|---|---|---|---|
|`-over`|현재 안 요소|←|이전 안 요소|
|`-out`|방금 떠난 요소<br /><sub>(이전 안 요소)</sub>|→|현재 안 요소|
...
##### `mouseover` <sub>(이벤트)</sub> 발생 <sub>(새 요소 안 진입)</sub> 시
- `mouseout` <sub>(이벤트)</sub> 반드시 발생 <sub>(이전 요소 밖 이동)</sub>
...
#### `HoverIntent(options)` <sub>(객체)</sub> 생성
