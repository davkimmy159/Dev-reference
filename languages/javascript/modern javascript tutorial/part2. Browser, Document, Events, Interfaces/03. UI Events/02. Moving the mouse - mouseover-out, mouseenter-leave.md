...
- `#parent` <sub>(요소)</sub> → `#child` <sub>(요소)</sub>
  1. `#parent` <sub>(요소)</sub> `mouseout` <sub>(이벤트)</sub>
  2. `#child` <sub>(요소)</sub> `mouseover` <sub>(이벤트)</sub>
- `#parent` <sub>(요소)</sub> ← `#child` <sub>(요소)</sub>
  1. `#parent` <sub>(요소)</sub> `mouseover` <sub>(이벤트)</sub>
  2. `#child` <sub>(요소)</sub> `mouseout` <sub>(이벤트)</sub>

|마우스 이동|발생 이벤트|
|---|---|
|`#parent` → `#child`|`#parent` `mouseout`|
||`#child` `mouseover`|
|`#parent` ← `#child`|`#parent` `mouseover`|
||`#child` `mouseout`|
...
##### `mouse[over·out]` 유사 · 차이 有
1. 자신 · 자손 요소 간 이동 시 미발생
2. 버블링 X
...
table.onmouseover = function(event) {
  /*
   ...
   - 새 요소 진입 직전
     - 항상 이전 요소 이탈
   currentElem (변수) 값 존재 시
   - <td> (요소) 이탈 X
     - mouseover (이벤트) 무시
   */
  if (currentElem) return;

  // 커서 빠르게 이동 시 <td> (요소) 생략 가능
  // - 외부 → <td> (요소) 내 자손 요소
  //   - <td> (요소) 찾기
...
##### 1. 이벤트 위임 사용
- 테이블 내 `<td>` <sub>(요소)</sub> 이동 이벤트 처리
  - `mouse[over·out]` <sub>(이벤트)</sub> 사용
  - `mouse[enter·leave]` <sub>(이벤트)</sub> X
    - 버블링 X
##### 2. `<td>` · 자손 <sub>(요소)</sub> 간 이동 제외
- `<td>` <sub>(요소)</sub> 간 이동 이벤트만 처리
...
##### 4. 커서 이탈 · `over` <sub>(함수)</sub> 종료 시
- `out` <sub>(함수)</sub> 실행

#### 커서 속도 계산 방법

##### `100ms` 마다 함수 실행
- 두 지점 <sub>(함수 실행 전 · 후)</sub> 사이 거리 계산
  - 거리 · 속도 비례
