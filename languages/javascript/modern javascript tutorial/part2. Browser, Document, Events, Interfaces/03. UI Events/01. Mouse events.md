...
이번 챕터에선 마우스 이벤트와 마우스 이벤트 <sub>(객체)</sub>의 프로퍼티에 대해 자세히 다루겠습니다.
...
`mousedown·mouseup`
- 요소 위에서 마우스 좌측 버튼을 누를 때, 마우스 버튼 누르고 있다가 뗄 때 발생합니다.
...
`click`
- 마우스 좌측 버튼을 사용해 동일한 요소 위에서 `mousedown` 이벤트와 `mouseup` 이벤트를 연달아 발생시킬 때 실행됩니다.

`dblclick`
- 동일한 요소 위에서 마우스 좌측 버튼을 빠르게 클릭할 때 발생합니다. 요즘엔 잘 쓰이지 않습니다.

`contextmenu`
- 마우스 우측 버튼을 눌렀을 때 발생합니다. 참고로 특별한 단축키를 눌러도 마우스 우측 버튼을 눌렀을 때처럼 컨텍스트 메뉴가 나타나게 할 수는 있지만 `contextmenu`라는 마우스 이벤트와 동일하진 않습니다.
...
단순하게 마우스 좌측 버튼을 클릭하더라도 `mousedown`, `mouseup`, 마지막으로 `click` 이벤트가 발생하죠.
...
`click` 이벤트는 마우스 좌측 버튼을, `contextmenu` 이벤트는 마우스 우측 버튼을 눌렀을 때 발생하기 때문에 `click`과 `contextmenu` 이벤트를 다룰 땐 보통 `button` 프로퍼티를 사용하지 않습니다.
...
상당수의 마우스는 좌측, 우측 버튼만 가지고 있기 때문에 이 마우스들이 만들어내는 `event.button` 값은 `0`이나 `2`가 됩니다. 터치를 지원하는 기기들도 사람이 해당 기기를 터치했을 때 유사한 이벤트를 만듭니다.
...
예시를 살펴봅시다. 아래 버튼은 Alt+Shift 키와 마우스 좌측 버튼을 함께 클릭했을 때만 동작합니다.
```html
...
<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **MacOS에선 Ctrl 대신 Cmd를 사용하세요!**

Windows와 Linux는 Alt, Shift, Ctrl 키를 지원합니다. 한편 MacOS에선 세 보조키 이 외에 Cmd라는 키를 추가로 지원하는데, Cmd 키에 해당하는 프로퍼티로는 `metaKey`가 있습니다.
...
물론 Mac 사용자가 Ctrl 키와 클릭을 동시에 하도록 강제할 수 있긴 합니다. 그런데 MacOS는 Ctrl 키와 마우스 좌측 버튼을 함께 누른 경우 마우스 우측 버튼을 누른 것으로 해석하기 때문에 문제가 발생합니다. Windows, Linux에선 `click` 이벤트로 해석될 수 있는 동작이 MacOS에선 `contextmenu` 이벤트로 해석되는 것이죠.
