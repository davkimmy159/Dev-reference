...
fires when an element uses setPointerCapture to enable capturing.
##### `gotpointercapture`
- 이벤트 바인딩 <sub>(`setPointerCapture` 메서드 호출)</sub> 시 발생
  - 요소 포인터 캡처링 적용

fires when the capture is released: either explicitly with releasePointerCapture call, or automatically on pointerup/pointercancel.
##### `lostpointercapture`
- 바인딩 제거 시 발생
  - 자동 · 수동 제거
