...
- 이벤트 <sub>(`pointerId`)</sub> → 요소 <sub>(`elem`)</sub> 바인딩
- 모든 `pointerId` 이벤트
...
##### 수동
- `elem.releasePointerCapture(pointerId)` 호출 시
...
##### 포인터 캡쳐링 사용
- `pointermove` <sub>(이벤트)</sub> → 손잡이 요소 바인딩
- Pointer capturing provides a means to bind  to `thumb` and avoid any such problems:
