...
##### 드래그 앤 드롭 포인터 이벤트 로깅
- 드래그 無 <sub>(누르기 → 떼기)</sub>
  1. `pointerdown`
  2. `pointermove` <sub>(n번)</sub>
  3. `pointerup`
- 드래그 有 <sub>(누르기 → 드래그 → 떼기)</sub>
  1. `pointerdown`
  2. `pointermove` <sub>(n번)</sub>
  3. `pointercancel`
...
#### 슬라이더 과제 <sub>([드래그 앤 드롭](./03.%20Drag'n'Drop%20with%20mouse%20events.md) · [마우스 이벤트](./01.%20Mouse%20events.md) 챕터)</sub>
