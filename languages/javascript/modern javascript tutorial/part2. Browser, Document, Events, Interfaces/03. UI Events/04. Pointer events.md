...
##### 포인터 캡쳐링 사용
- `pointermove` <sub>(이벤트)</sub> → 손잡이 <sub>(요소)</sub> 바인딩
  - 타 이벤트 핸들러 동작 방지
- `pointerdown` 핸들러 내
  - 메서드 호출
```javascript
...
```
- 이후 포인터 이벤트 <sub>(`pointer[up·cancel]` 전)</sub>
  - 항상 손잡이 <sub>(`thumb` 요소)</sub> 대상 발생
- `pointerup` <sub>(이벤트)</sub> 발생 <sub>(드래깅 종료)</sub> 시
  - 바인딩 자동 제거
