...
- 시작점 · 끝점 동일 여부 <sub>(범위 내 내용 無)</sub>
  - `false`

##### `commonAncestorContainer`
- 최근접 공통 조상 <sub>(범위 내 모든 노드)</sub>
  - `<p>` <sub>(요소)</sub>

#### 메서드

##### `set[Start·End](node, offset)`
- `node` 내 `offset` 위치 <sub>(시작점 · 끝점 설정)</sub>

##### `set[Start·End]Before(node)`
- `node` 직전 위치 <sub>(시작점 · 끝점 설정)</sub>

##### `set[Start·End]After(node)`
- `node` 직후 위치 <sub>(시작점 · 끝점 설정)</sub>
