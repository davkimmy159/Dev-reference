...
- 해당 요소 선택 시작 방지
- 타 요소 선택 시작 시 해당 요소 포함 가능
- That’s convenient when there’s another event handler on the same action that triggers the select (e.g. `mousedown`).
- 다수 선택 유발 이벤트 핸들러 설정 시 유용
  - ex\) `mousedown` <sub>(이벤트)</sub> 핸들러 등
- So we disable the selection to avoid conflict, still allowing `elem` contents to be copied.
- 선택 비활성화
  - 충돌 방지
  - 해당 요소 복사 가능
```html
