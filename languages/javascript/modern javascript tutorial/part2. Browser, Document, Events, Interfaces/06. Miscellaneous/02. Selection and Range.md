...
- `"select"`
  - the newly inserted text will be selected.
- `"start"`
  - the selection range collapses just before the inserted text (the cursor will be immediately before it).
- `"end"`
  - the selection range collapses just after the inserted text (the cursor will be right after it).
"preserve" – attempts to preserve the selection. This is the default.
- replace a range of text with the new text.
- 기존 텍스트 범위 → 새 텍스트 범위 교체
- 기존 범위 내용 <sub>(텍스트)</sub> → 새 텍스트 교체
- `replacement`
  - 교체삽입될 새 텍스트
- `[start]` · `[end]`
  - 시작점 · 끝점
