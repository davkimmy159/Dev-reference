...
##### `anchorOffset`
- the offset in `anchorNode` where the selection starts,

##### `focusNode`
- the node where the selection ends,

##### `focusOffset`
- the offset in `focusNode` where the selection ends,

##### `isCollapsed`
- `true` if selection selects nothing (empty range), or doesn't exist.

##### `rangeCount`
- count of ranges in the selection, maximum `1` in all browsers except Firefox.
