...
<script>
  function ready() {
    ...
    // 이미지 크기: 0x0
    // - 이벤트 발생 시점 이미지 로드 X
    alert(`이미지 크기: ${img.offsetWidth}x${img.offsetHeight}`);
  }
...
#### [`navigator.sendBeacon(url, data)`](https://w3c.github.io/beacon/) <sub>(메서드)</sub>
#### [`navigator.sendBeacon(url, data)`](https://w3c.github.io/beacon/)
##### [`navigator.sendBeacon`](https://w3c.github.io/beacon/) <sub>(메서드)</sub>
- 백그라운드 전송 수행
  - 타 페이지 전환 시 지연 미발생
  - 분석 정보 정상 전송
- 요청 전송
  - POST 메서드
- 전송 대상 <sub>(보통 문자열 형태 객체)</sub>
  - 문자열
  - 폼
  - 기타 포맷
- 전송 데이터 용량
...
### `onbeforeunload` <sub>(이벤트)</sub>

##### `window` <sub>(전역 객체)</sub> 발생
- `window.onbeforeunload` <sub>(프로퍼티)</sub>
- `addEventListener` <sub>(메서드)</sub>
...
##### 방법
1. `false` 반환
2. 비어있지 않은 문자열 반환 <sub>([명세서](https://html.spec.whatwg.org/#unloading-documents) 미권장)</sub>
    - 역사적 이유
    - 구식 브라우저
      - 반환 문자열 표시
    - 모던 브라우저
