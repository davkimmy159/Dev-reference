...
- 얼럿창 출력 <sub>(`DOMContentLoaded`)</sub>
```html
<p> … 스크립트 앞 콘텐츠 … </p>
...
##### 지연 <sub>(`defer`)</sub> · 비동기 <sub>(`async`)</sub> 스크립트
- 다운로드 시 페이지 렌더링 중단 X
  - 대기 시간 ↓
...
||순서|`DOMContentLoaded` <sub>(이벤트)</sub>|
|---|---|---|
|비동기<br /><sub>(`defer`)</sub>|문서 추가 순서|- HTML 문서 다운로드 · 파싱 완료 후<br />`- DOMContentLoaded` <sub>(이벤트)</sub> 발생 전|
|지연<br /><sub>(`async`)</sub>|다운로드 완료 순서<br /><sub>(load-first order)</sub>|HTML 문서 다운로드 완료 전 · 후<br />- 스크립트 크기 少<br />- 캐싱 처리<br />- HTML 문서 길이 大|
