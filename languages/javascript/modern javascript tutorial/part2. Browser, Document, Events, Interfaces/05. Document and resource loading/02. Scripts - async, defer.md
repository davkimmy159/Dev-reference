...
##### 순서
1. 페이지 콘텐츠 즉시 출력 <sub>(DOM 트리 완성)</sub>
2. 지연 스크립트 실행
...
- 페이지 내 스크립트들 확인 후
  - 병렬적 다운로드 실행
- 다운로드 완료 순서
  1. 소용량 파일 <sub>(`small.js`)</sub>
  2. 대용량 파일 <sub>(`long.js`)</sub>
- 실행 순서 <sub>(문서 추가 순서 · 명세서 정의)</sub>
  1. 대용량 파일 <sub>(`long.js`)</sub>
  2. 소용량 파일 <sub>(`small.js`)</sub>
