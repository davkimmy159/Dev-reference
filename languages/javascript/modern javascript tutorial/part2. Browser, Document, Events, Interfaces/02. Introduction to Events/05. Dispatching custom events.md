...
- `hide()` <sub>(함수)</sub>
  - `hide` <sub>(이벤트)</sub> 실행
  - 다른 코드들에 이벤트 실행 여부 전달
- `rabbit.addEventListener('hide', …)`
- 사용하면 어떤 핸들러에서도 `"hide"`를 리스닝 할 수 있습니다. 그리고 필요하다면 `event.preventDefault()`를 사용해 `"hide"` 이벤트의 기본 동작을 취소할 수 있습니다. 이렇게 기본동작이 취소되면 토끼가 화면에서 사라지지 않습니다.
```html
<pre id="rabbit">
  |\   /|
   \|_|/
