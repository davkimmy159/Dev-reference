...
- ball width/height must be known at the time we access ball.offsetWidth.
- Should be specified in HTML or CSS.

##### 참고
- 공 <sub>(요소)</sub> `offset[*]` <sub>(기하 프로퍼티)</sub> 접근 시
  - `width` · `height` <sub>(프로퍼티)</sub> 값 필요
    - 명시적 값 설정 <sub>(HTML · CSS)</sub> 필요
...
##### 1. `position: absolute` <sub>(CSS)</sub>
- 메시지 본문 <sub>(`<pane>` 요소)</sub>
  - `position: relative`
- 좀 더 유연한 방법

##### 2. `float: right` <sub>(CSS)</sub>
- 버튼 · 텍스트
  - 절대 겹치지 않음
