...
##### `left` <sub>(클릭 좌표)</sub> 계산 <sub>(운동장 요소 기준)</sub>
```javascript
// 창 기준 클릭 좌표 - 운동장 (요소) 좌측 좌표 - 테두리 두께
...
```javascript
// 창 기준 클릭 좌표 - 운동장 (요소) 좌측 좌표 - 테두리 두께 - 공 (요소) 너비 절반
let left = event.clientX - fieldCoords.left - field.clientLeft - ball.offsetWidth / 2;
```

##### `top` <sub>(클릭 좌표)</sub> 계산 <sub>(운동장 요소 기준)</sub>
- `left` <sub>(클릭 좌표)</sub> 계산 동일
```javascript
// 창 기준 클릭 좌표 - 운동장 (요소) 상단 좌표 - 테두리 두께 - 공 (요소) 높이 절반
