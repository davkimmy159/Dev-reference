...
##### 몇몇 이벤트 버블
- 타깃 <sub>(이벤트 발생 요소)</sub> → `window` <sub>(객체)</sub>
  - 거쳐가는 각 노드 모두 이벤트 발생
...
##### 캡쳐링 단계
- 최상위 노드 <sub>(`document` · `window`)</sub> → `event.target`
- `addEventListener(…, true)` 할당 핸들러 동작
  - `(…, true)`
    - `(…, { capture: true })` 축약형
...
##### 버블링 단계
- `event.target` → 최상위 노드
까지 전달되면서 각 요소에 `on<event>`로 할당한 핸들러와 `addEventListener`로 할당한 핸들러를 동작시킵니다. `addEventListener`로 할당한 핸들러 중, 세 번째 인수가 없거나 `false`, `{capture: false}`인 핸들러만 호출됩니다.
