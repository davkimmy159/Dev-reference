...
##### 버블링 중단 메서드 <sub>(이벤트 객체)</sub> 비교 <sub>(2가지)</sub>
- `stopPropagation()`
  - 핸들러 중 하나 버블링 중단
  - 타 핸들러 여전히 버블링 진행
...
<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **버블링 중단 지양 <sub>(필요 시만 중단)</sub>**
...
- 상위 메뉴 <sub>(요소)</sub> 핸들러 <sub>(`click` 이벤트)</sub>
  - `stopPropagation()` 적용
    - 서브 메뉴 `click` <sub>(이벤트)</sub> 미처리

##### b. 행동 패턴 분석 <sub>(사람들 페이지 내 클릭 위치 등)</sub> 실행
