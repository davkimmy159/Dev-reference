...
##### `event.defaultPrevented` <sub>(프로퍼티)</sub> 값
- 기본 동작
  - 막은 경우
    - `true`
  - 막지 않은 경우
...
##### `<a>` <sub>(요소)</sub> · 기본 동작 막기 조합 주의점
- '새 창에서 열기' <sub>(기능)</sub>
  - 많은 사용자들 사용
- 링크 같은 `button` <sub>(요소)</sub>
  - `<a>` 관련 기능 작동 X
...
##### HTML 속성 할당 핸들러
1. `on*` <sub>(속성)</sub> 읽기 <sub>(브라우저)</sub>
...
- 사용자 : `<a>` <sub>(링크 요소)</sub> 클릭 시
  - `#largeImg` <sub>(요소)</sub> `src` <sub>(속성)</sub> 값
    - 섬네일 `href` <sub>(속성)</sub> 값 변경
