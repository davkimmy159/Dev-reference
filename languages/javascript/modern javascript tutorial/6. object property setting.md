...
- 프로퍼티 미존재 시 인수 넘겨받은 정보 이용해 새 프로퍼티 생성
...
### `enumerable` 플래그
- 객체 내장 메서드 `toString` 열거 불가능 <sub>(non-enumerable)</sub>
  - `for…in` 사용 시 안 나타남
...
<img class="icon" src="./images/commons/icons/circle-exclamation-solid.svg" /> **'non-configurable' vs 'non-writable'**
...
- 한 객체 내 프로퍼티 전체 대상 제약사항 설정
  - `Object.preventExtensions(obj)`
    - 객체 새 프로퍼티 추가 X
  - `Object.seal(obj)`
    - 새 프로퍼티 추가 · 기존 프로퍼티 삭제 X
    - 프로퍼티 전체 `configurable: false` 설정 동일 효과
  - `Object.freeze(obj)`
    - 새 프로퍼티 추가 · 기존 프로퍼티 삭제 · 수정 X
...
### getter · setter
- `getter` <sub>(획득자)</sub> · `setter` <sub>(설정자)</sub> 메서드
...
```
- 프로퍼티
  - 접근자 프로퍼티 <sub>(`get` · `set`)</sub> · 데이터 프로퍼티 <sub>(value)</sub> 중 한 종류만 가능
  - 둘 다 설정
    - 에러 발생
```javascript
...
### `getter` · `setter` 똑똑하게 활용
- 실제 프로퍼티 값 감싸는 래퍼 처럼 사용
  - 프로퍼티 값 원하는 대로 통제
```javascript
let user = {
  get name() {
