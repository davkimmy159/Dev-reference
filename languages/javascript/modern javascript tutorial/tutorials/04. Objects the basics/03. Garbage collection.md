...
2. 루트가 참조하고 있는 모든 객체 방문 후 mark <sub>(기억)</sub>

![garbage-collection-2](../../images/04/03/garbage-collection-2.svg)

3. mark <sub>(기억)</sub> 된 모든 객체 방문 후 그 객체들이 참조하는 객체 mark <sub>(기억)</sub>
    - 한번 방문한 객체 전부 mark <sub>(기억)</sub>
    - 같은 객체 재방문 X
...
5. mark <sub>(기억)</sub> 되지 않은 모든 객체 메모리에서 삭제
