...
```
위 예시에서 커스텀 객체 range는 반복 가능하고, 제너레이터 *[Symbol.iterator]엔 값을 나열해주는 로직이 구현되어 있습니다.

지금 상태에서 제너레이터에 비동기 동작을 추가하려면, Symbol.iterator를 async Symbol.asyncIterator로 바꿔야 합니다.
