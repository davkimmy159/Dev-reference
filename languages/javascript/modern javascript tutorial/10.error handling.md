...
try {
  {{{{{{{{{{{{
      ...
      alert("유효하지 않은 코드 → 자바스크립트 엔진 코드 이해 X");
}
```
- 자바스크립트 엔진
  - 코드 읽은 후 실행
