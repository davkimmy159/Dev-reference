...
##### 생성자 <sub>(함수)</sub> 호출 시 구분
- 일반 클래스 생성자 <sub>(함수)</sub>
- 상속 클래스 생성자 <sub>(함수)</sub>
  - `[[ConstructorKind]]: "derived"`
    - 특수 내부 프로퍼티

#### 일반 · 상속 클래스 생성자 <sub>(함수)</sub> 차이

##### 일반 클래스 생성자 <sub>(함수)</sub> 실행 <sub>(`new`)</sub> 시
1. `{}` <sub>(빈 객체)</sub> 생성
2. `this` 에 객체 할당

##### 상속 클래스 생성자 <sub>(함수)</sub> 실행 <sub>(`new`)</sub> 시
1. `{}` <sub>(빈 객체)</sub> 생성
