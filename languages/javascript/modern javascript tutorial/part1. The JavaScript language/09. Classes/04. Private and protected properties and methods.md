...
#### 객체 필드 <sub>(프로퍼티 · 메서드)</sub> 종류 <sub>(2가지)</sub>
...
#### `CoffeeMachine` <sub>(클래스)</sub>

##### 수정 전
- 객체 내 필드 종류
  - `public`
- 입니다. 손쉽게 `waterAmount`와 `power`를 읽고 원하는 값으로 변경하기 쉬운 상태이죠.

이제 `waterAmount`를 `protected`로 바꿔서 `waterAmount`를 통제해 보겠습니다. 예시로 `waterAmount`를 `0` 미만의 값으로는 설정하지 못하도록 만들어 볼 겁니다.
```javascript
class CoffeeMachine {
  waterAmount = 0; // 물 양
