에러 처리 <sub>(`try…catch`)</sub>
====

##### 다양한 에러 원인
- 실수
- 예상치 못한 사용자 입력
- 잘못된 서버 응답
- 기타 등등

##### 에러 결과
- 스크립트 즉시 중단
- 콘솔 내 에러 출력

### `try…catch` 문법

##### 기능
- 스크립트 중단 방지
- 에러 진행 방지
  - 더 합당한 작업 실행

##### 주요 구성 블록 <sub>(2가지)</sub>
- a. `try{…}`
- b. `catch(…){…}`
```javascript
try {

  /* …코드… */

} catch (err) {

  /* …에러 처리… */
}
```

#### 동작 알고리즘

##### 1. 코드 실행
- `try{…}` <sub>(블록)</sub>

##### 2. 에러 無
- `try{…}` <sub>(블록)</sub> 내 코드 실행 완료
- `catch(…){…}` <sub>(블록)</sub> 미실행

##### 3. 에러 有
- `try{…}` <sub>(블록)</sub> 내 코드 실행 중단
- `catch(err){…}` <sub>(블록)</sub> 실행 <sub>(제어 흐름 이동)</sub>
  - `err` <sub>(변수)</sub>
    - 에러 <sub>(정보)</sub> 객체 포함

![try-catch-flow](../../images/01/10/01/try-catch-flow.svg)

##### 에러 無 예시
```javascript
try {
  alert('try 블록 시작'); // 정상 실행

  // …코드 정상 실행…

  alert('try 블록 끝');  // 정상 실행

//catch 블록 무시
} catch(err) {
  alert('에러 無 → catch 무시');
}
```

##### 에러 有 예시
```javascript
try {
  alert('try 블록 시작'); // 정상 실행

  lalala;              // 에러 (미정의 변수)

  alert('try 블록 끝');  // 미실행 (도달 X)

// 에러 시 catch 블록 즉시 실행
} catch(err) {
  alert('에러');
}
```

<br />

<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **`try…catch` 동작 대상 <sub>(런타임 에러)</sub>**

##### 실행 가능 <sub>(유효)</sub> 코드 <sub>(문법 문제 無)</sub> 대상 동작
- 실행 불가능 <sub>(무효)</sub> 코드 대상 동작 X
  - 부적절한 문법
```javascript
try {
  {{{{{{{{{{{{ // 잘못된 문법
} catch(e) {
  alert("무효 코드 → JS 엔진 코드 이해 불가능");
}
```

#### 에러 구분

##### 파싱 에러
- 코드 읽기 중 발생
- 엔진 해당 코드 이해 불가능
  - 코드 내 복구 불가능

##### 런타임 에러 <sub>(≒ 예외)</sub>
- 코드 실행 중 발생
- `try…catch` 동작 대상

<br />

<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **`try…catch` 동기 동작**

##### 예약 실행 코드 <sub>(`setTimeout` 등)</sub> 내 에러 처리 불가능
- `try…catch` 종료 후 예약 함수 실행
```javascript
try {
  setTimeout(function() {
    noSuchVariable; // 스크립트 중단
  }, 1000);
} catch (e) {
  alert( "작동 중단" );
}
```
- 예약 함수 내 `try…catch` 구현 필요
```javascript
setTimeout(function() {
  try {
    noSuchVariable; // try…catch 내 에러 처리
  } catch {
    alert( "에러 처리 성공" );
  }
}, 1000);
```

<br />

### 에러 객체

##### 에러 상세 정보 저장
- 에러 시 생성 후
  - `catch(…){…}` <sub>(블록)</sub> 인수 전달
```javascript
try {
  …
} catch(err) { // err (에러 객체)
  …            // - 인수명 변경 가능
}
```

##### 주요 프로퍼티
- `name` <sub>(문자열)</sub>
  - 에러명 <sub>(`'ReferenceError'` 등)</sub>
- `message` <sub>(문자열)</sub>
  - 에러 상세 정보

##### 이 외 프로퍼티 <sub>(非표준 · 대다수 호스트 환경 지원)</sub>
- `stack` <sub>(문자열 · 사용 빈도 多)</sub>
  - 현재 호출 스택 <sub>(에러 유발 중첩 호출 순서)</sub>
  - 디버깅 용도
```javascript
try {
  lalala; // 에러 (변수 미정의)
} catch(err) {
  err.name;    // ReferenceError
  err.message; // lalala is not defined
  err.stack;   // ReferenceError: lalala is not defined at … (호출 스택)

  // 에러 전체 표시 ("name: message" 형태)
  err;         // ReferenceError: lalala is not defined
}
```

### 선택적 `catch` <sub>(문)</sub> 바인딩

<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **최근 명세서 추가**

##### 구식 브라우저
- 폴리필 필요

<br />

##### `catch` <sub>(문)</sub> 인수 <sub>(에러 객체)</sub> 생략 가능
- 에러 상세 정보 불필요 시 생략
```javascript
try {
  …
} catch { // 인수 (에러 객체) 생략
  …
}
```

### `try…catch` 사용

##### [`JSON.parse`](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse) <sub>(메서드)</sub>
- JSON <sub>(인코딩 값)</sub> → 값 변환
- 주요 용도
  - 네트워크 <sub>(서버 등)</sub> 수신 데이터 디코딩
```javascript
// 네트워크 (서버 등) 수신 데이터 (문자열)
let json = '{"name":"John", "age": 30}';

// JSON (인코딩 문자열) → 객체 변환
let user = JSON.parse(json);
/*  user = {
  name: "John",
  age: 30
}
 */
```

##### 부적절한 JSON 형식
- `JSON.parse` <sub>(메서드)</sub> 실행 시 에러
  - 미처리 시 스크립트 중단 발생
  - `try…catch` 사용한 처리 필요
```javascript
// 부적절한 JSON 형식
let json = "{bad json}";

try {
  let user = JSON.parse(json); // 에러

  user.name; // 미실행 (도달 X)

// 에러 시 catch (문) 제어 흐름 이동
} catch (e) {
  "데이터에 에러가 있어 재요청을 시도합니다.";
  e.name;
  e.message;
}
```

##### `catch(…){…}` <sub>(블록)</sub>
- 에러 후 구체적인 동작 실행 가능
  - 새 네트워크 요청 전송
  - 사용자 대안 제안
  - 로깅 장치 에러 정보 전송
  - 기타 등등

### 직접 에러 생성 후 던지기

##### 문법 정상 · 불완전 데이터
- `JSON.parse` <sub>(메서드)</sub> 정상 실행 <sub>(문법 정상)</sub>
  - `name` <sub>(프로퍼티)</sub> 無
    - 추후 에러 유발
```javascript
// 불완전 데이터
// - name (프로퍼티) 無
let json = '{"age": 30}';

try {

  // JSON → 객체 변환 시 에러 X (문법 정상)
  let user = JSON.parse(json);

  // name (프로퍼티) 無 (추후 에러 유발)
  user.name; // "" (빈 문자열)

} catch (e) {
  "실행되지 않습니다.";
}
```

#### `throw` <sub>(연산자)</sub>

##### 에러 생성
```javascript
throw <error object>
```

##### 에러 객체
- 모든 값 사용 가능 <sub>(원시형 자료 등)</sub>
- 객체 <sub>(표준 프로퍼티 구현)</sub> 권장 <sub>(내장 에러 객체 호환)</sub>
  - `name` <sub>(문자열)</sub>
    - 에러명
  - `message` <sub>(문자열)</sub>
    - 에러 상세 정보

##### 표준 에러 객체 생성자
- `Error`
- `SyntaxError`
- `ReferenceError`
- `TypeError`
- 기타 등
```javascript
let error1 = new Error(message);
let error2 = new SyntaxError(message);
let error3 = new ReferenceError(message);
…
```

##### 내장 에러 객체 <sub>(내장 생성자 사용)</sub>
- `name` <sub>(프로퍼티)</sub>
  - 내장 생성자명
- `message` <sub>(프로퍼티)</sub>
  - 내장 생성자 인수
```javascript
let error = new Error("이상한 일이 발생했습니다. o_O");

error.name;    // Error
error.message; // 이상한 일이 발생했습니다. o_O
```

##### `SyntaxError` <sub>(에러)</sub>
- `JSON.parse` <sub>(메서드)</sub> 실행 후
  - 부적절한 JSON 형식 변환 시 발생
```javascript
try {
  JSON.parse("{잘못된 형식의 json o_O}");
} catch(e) {
  e.name;    // SyntaxError
  e.message; // Unexpected token b in JSON at position 2
}
```

##### 문법 정상 · 불완전 데이터 → 에러 간주
- `JSON.parse` <sub>(메서드)</sub> 실행 시
  1. 에러 생성 후 던지기 <sub>(`throw` 연산자 )</sub>
  2. 에러 처리 <sub>(`try…catch`)</sub>
```javascript
// 불완전 데이터
// - name (프로퍼티) 無
let json = '{"age": 30}';

try {

  // JSON → 객체 변환 시 에러 X (문법 정상)
  let user = JSON.parse(json);

  // name (프로퍼티) 유무 여부 확인
  if (!user.name) {

    /* throw (연산자)
     SyntaxError (에러 객체) 생성
     - 생성자 인수 → message (에러 객체 내 프로퍼티) 할당
       - JS 자체 에러 생성 방식 동일
     try (문) 즉시 실행 중단
       - catch (문) 실행 (제어 흐름 이동)
     */
    throw new SyntaxError("불완전 데이터 (이름 없음)");
  }

  user.name; // 미실행 (도달 X)

// catch(…){…} (블록) 내 모든 에러 처리 
} catch(e) {
  e.message; // 불완전 데이터 (이름 없음)
}
```

### 에러 다시 던지기

##### 다양한 에러 발생 가능
- 미정의 변수 사용
- 기타 등등
```javascript
let json = '{"age": 30}';  // 불완전 데이터

try {
  user = JSON.parse(json); // let 미선언
  …
} catch(err) {
  err; // ReferenceError: user is not defined
}
```

##### `catch`
- `try{…}` <sub>(블록)</sub> 내 모든 에러 처리
- 예상치 못한 에러 (미정의 변수 등)
  - 잡기는 함
  - 항상 특정 내용 <sub>(`"JSON Error"`)</sub> 표시
    - 에러 종류 무관
- 종류 관계없이 동일 방식 에러 처리
  - 디버깅 난이도 ↑

##### 다시 던지기 <sub>(rethrowing)</sub>
- `catch`
  - 알고 있는 에러만 처리
  - 나머지 '다시 던짐'

##### 규칙 순서
1. `catch`
    - 모든 에러 받음
2. `catch(err){…}` <sub>(블록)</sub> 안
    - `err` <sub>(에러 객체)</sub> 분석
3. 에러 처리 방법 알지 못함
    - `throw err` <sub>(다시 던지기)</sub>

<br />

##### 일반적인 에러 타입 확인
- `instanceof` <sub>(연산자)</sub>
```javascript
try {
  user = { … };
} catch(err) {
  if (err instanceof ReferenceError) {

  // 미정의 변수 접근
  // - ReferenceError 발생
    alert('ReferenceError');
  }
}
```

##### `err.name` <sub>(프로퍼티)</sub>
- 에러 클래스명
- 모든 기본형 에러 보유
- == `err.constructor.name`

##### 에러 다시 던지기
- `catch(…){…}` <sub>(블록)</sub>
  - `SyntaxError` 만 처리
```javascript
// 불완전 데이터
let json = '{"age": 30}';

try {

  let user = JSON.parse(json);

  if (!user.name) {
    throw new SyntaxError("불완전 데이터 (이름 없음)");
  }

  // 예상치 못한 에러
  blabla();

  alert( user.name );

} catch(e) {

  if (e instanceof SyntaxError) {
    alert( "JSON Error: " + e.message );

  // 이 외 예기치 못한 에러
  } else {

    // 다시 던지기
    throw e;
  }
}
```

##### 다시 던져진 에러 <sub>(`catch(…){…}` 블록)</sub>
- `try…catch` 밖으로 던져짐

#### 외부 `try…catch`
- 존재 시
  - 던져진 에러 잡음
- 부재 시
  - 스크립트 중단

#### `catch(…){…}` <sub>(블록)</sub>
- 어떻게 다룰지 아는 에러만 처리
- 알 수 없는 에러 건너뜀 <sub>(처리 X)</sub>

##### 다시 던져진 예상치 못한 에러 처리
```javascript
function readData() {
  let json = '{"age": 30}';

  try {
    …
    blabla(); // 에러
  } catch (e) {
    …
    if (!(e instanceof SyntaxError)) {

      // (알 수 없는) 에러 다시 던지기
      throw e;
    }
  }
}

try {
  readData();
} catch (e) {

  // 다시 던져진 에러
  // - 밖의 try…catch 잡음
  alert( "External catch got: " + e );
}
```

##### `readData` <sub>(함수)</sub>
- `SyntaxError` 만 처리

##### 함수 밖 `try…catch`
- 예상치 못한 에러 처리

### `try…catch…finally`

##### `finally`
- 에러 無
  - `try` 실행 후 실행
- 에러 有
  - `catch` 실행 후 실행
- `try…catch` 확장
```javascript
try {
   … 코드 실행 …
} catch(e) {
   … 에러 핸들링 …
} finally {
   … 항상 실행 …
}
```
```javascript
try {
  alert( 'try 블록 시작' );
  if (confirm('에러를 만드시겠습니까?')) {
    이상한_코드();
  }
} catch (e) {
  alert( 'catch' );
} finally {
  alert( 'finally' );
}
```

##### 코드 실행 경로
- `"에러를 만드시겠습니까?"`
1. Yes
    - `try` → `catch` → `finally`
2. No
    - `try` → `finally`

##### `finally` <sub>(절)</sub>
- 무언가 실행 후
  - 결과 상관없이 실행

##### ex\) `fib(n)` <sub>(피보나치 함수)</sub>
- 연산 시간 측정
  - 함수 실행 전 측정 시작
  - 함수 실행 후 측정 종료
- 함수 실행 도중 에러 경우
  - 정수 아닌 수 · 음수 입력 등
- `finally`
  - 결과 상관없이 연산 시간 측정
```javascript
let num = +prompt("양의 정수를 입력해주세요.", 35)

let diff, result;

function fib(n) {
  if (n < 0 || Math.trunc(n) != n) {
    throw new Error("음수나 정수가 아닌 값은 처리할 수 없습니다.");
  }
  return n <= 1 ? n : fib(n - 1) + fib(n - 2);
}

let start = Date.now();

try {
  result = fib(num);
} catch (e) {
  result = 0;
} finally {
  diff = Date.now() - start;
}

alert(result || "에러");

alert( `연산 시간: ${diff}ms` );
```

##### 프롬프트 창 입력값
- `35`
  - `try` 다음 `finally` 정상 실행
    - 연산 시간 계산 정상 실행
- `-1`
  - 에러
    - 연산 시간 : `0ms`
- 함수 종료
  - `return` · `throw` <sub>(문)</sub> 도달
  - `finally` <sub>(절)</sub>
    - 두 경우 모두 처리

<br />

<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`try…catch…finally` 내 변수 : 지역 변수**

##### 위 예시 `diff` · `result` <sub>(변수)</sub>
- `try…catch` 이전 선언

##### `try{…}` <sub>(블록)</sub> 내 선언 변수
- 지역 변수 <sub>(`{…}` 블록 내에서만 유효)</sub>

<br />

<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`finally` · `return`**

##### `finally` <sub>(절)</sub> 실행
- `try…catch` 절 빠져나가는 모든 경우
- `return` 사용해 명시적으로 빠져나가는 경우
- `try{…}` <sub>(블록)</sub> 내 `return` 사용 시
  - 외부 코드로 값 반환 전 실행
```javascript
function func() {
  try {
    return 1;
  } catch (e) {
    …
  } finally {
    alert( 'finally' );
  }
}

// finally (절) alert 실행 후 실행
alert( func() );
```

<br />

<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`try…finally 구문`**

- `catch` <sub>(절)</sub> 생략
- 상황 따라 유용
- `try…finally` 내 에러 처리 X
  - 프로세스 마무리 확인 등
```javascript
function func() {
  // 끝맺음 있어야 하는 어떤 프로세스
  // - 무언가 측정 등
  try {
    …
  } finally {
    // 스크립트 중단되더라도 완료
  }
}
```

##### `try` 내 발생 에러
- 항상 밖으로 떨어져 나옴

##### `finally`
- 실행 흐름이 함수 떠나기 전 실행

### 전역 `catch`

<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **호스트 환경 확인**

- 코어 JS X

<br />

##### 상황 가정
- `try…catch` 외부
  - 치명적인 에러 발생
    - 스크립트 중단
- 대처 방법
  - 어딘가 에러 내역 기록
  - 사용자에게 에러 알림
  - 기타 등등
- JS 명세서
  - 치명적인 에러 대응 방법 X
- `try…catch` 미처리 에러 잡기
  - 아주 중요
- 대다수 JS 호스트 환경
  - 자체 에러 처리 기능 제공

##### Node.js
- `process.on("uncaughtException")`

##### 브라우저
- `window.onerror` <sub>(함수 할당)</sub>
  - 예상치 못한 에러 발생 시 함수 실행
```javascript
window.onerror = function(message, url, line, col, error) {
  …
};
```
- `message`
  - 에러 메시지
- `url`
  - 에러 스크립트 url
- `line` · `col`
  - 에러 위치 줄 · 열
- `error`
  - 에러 객체
```html
<script>
  window.onerror = function(message, url, line, col, error) {
    alert(`${message}\n At ${line}:${col} of ${url}`);
  };

  function readData() {

    // 미정의 함수 (에러 위치)
    badFunc();
  }

  readData();
</script>
```

##### `window.onerror` <sub>(전역 핸들러)</sub>
- 중단된 스크립트 복구 목적
  - 드물게 사용
- `window.onerror` 만으로 스크립트 복구
  - 사실상 불가능
- 개발자에게 에러 메시지 보내는 용도
- 에러 로깅 관련 상용 서비스
  - https://errorception.com
  - http://www.muscula.com
  - 기타 등등

#### 상용 서비스 동작 프로세스

##### 1. 서비스 가입
- JS 파일 · 스크립트 url 받음
- 페이지에 파일 · url 삽입

##### 2. 파일 · url 동작
- `window.onerror` <sub>(커스텀 함수)</sub> 설정

##### 3. 에러 발생 시
- 커스텀 함수 동작
  - 에러 내용 담아 서비스에 네트워크 요청 보냄

##### 4. 서비스 사이트 로그인
- 기록된 에러 확인

<br />

요약
====

##### `try…catch`
- 런타임 에러 처리
- `try`
  - 코드 실행
- `catch`
  - 에러 발생 시 잡아냄

##### 문법
```javascript
try {

  // 에러 가능성 有 코드 실행

} catch(err) {

  // 에러 발생 시 실행
  // - err (에러 객체)

} finally {

  // try · catch 이후 실행
  // - 에러 여부 무관

}
```
- `try…catch`
- `try…catch…finally`
- `try…finally`

#### 에러 객체 프로퍼티

##### `message`
- 에러 메시지
  - 사람이 읽을 수 있는 형태

##### `name`
- 에러명 <sub>(문자열)</sub>
  - 에러 생성자명

##### `stack`
- 非표준 <sub>(대다수 호스트 환경 지원)</sub>
- 에러 발생 순간 스택

##### 에러 객체 불필요 시
- 인수 <sub>(`err` 객체)</sub> 생략
  - `catch(err) {}` → `catch {}`

##### `throw` <sub>(연산자)</sub>
- 에러 직접 생성
- 인수
  - 모든 것 가능
  - 보통 에러 객체 <sub>(내장 `Error` 클래스 상속)</sub>

##### 다시 던지기
- 에러 처리 중요 패턴
- `catch(…){…}` <sub>(블록)</sub>
  - 보통 예상 가능 · 아는 에러 처리
  - 예상치 못한 에러
    - 다시 던지기

##### 대다수 호스트 환경
- '전역' 에러 핸들러 지원 <sub>(`try…catch` X)</sub>
  - '떨어져 나온' 에러 잡기 가능
- ex\) 브라우저 환경 전역 에러 핸들러
  - `window.onerror`

<br />

## <img src="../../images/commons/icons/circle-check-solid.svg" /> 과제

### `finally` 아니면 코드만?

#### 두 코드 조각 비교

##### 1. `try…catch…finally`
- `try…catch` 이후 코드
  - `finally` 내 실행
```javascript
try {
  작업
} catch (e) {
  에러 핸들링
} finally {
  작업 내역 삭제
}
```

##### 2. `try…catch`
- `try…catch` 이후 코드
  - 별도 실행
```javascript
try {
  작업
} catch (e) {
  에러 핸들링
}

작업 내역 삭제
```

##### 현재 상황
- 작업 후 초기화 실행 필요
  - 에러 유무 무관

##### 참고
- `finally` 사용 이점 존재 시
  - 이점 드러나는 예시 제시

<br />

<img src="../../images/commons/icons/circle-answer.svg" />

#### `try…catch` 내 '빠져나오게 하는' 코드
- 함수 행동 변화시킴
- ex\)
  - `return` <sub>(문)</sub>
  - `throw` <sub>(연산자)</sub>

##### `finally` <sub>(절)</sub>
- 모든 `try…catch` 종료 상황 하 실행
  - `return` <sub>(문)</sub> · `throw` <sub>(연산자)</sub> 포함
- 실행 시점
  - `try…catch` 종료 후
  - 함수 호출 코드 : 제어권 갖기 직전
- 초기화 작업 수행 보장
- 단순히 함수 끝에 초기화 코드 추가 시
  - 초기화 코드 미실행 가능성 有

##### `return` <sub>(문)</sub>
```javascript
function f() {
  try {
    alert('시작');
    return "결과";
  } catch (e) {
    …
  } finally {
    alert('초기화!');
  }
}

f();
```

##### `throw` <sub>(연산자)</sub>
```javascript
function f() {
  try {
    alert('시작');
    throw new Error("에러");
  } catch (e) {
    …
    if ("에러를 핸들링 할 수 없다면") {
      throw e;
    }

  } finally {
    alert('초기화!')
  }
}

f();
```
