...
- [require.js](http://requirejs.org/) <sub>(라이브러리)</sub>
...
/*  📁 main.js (파일) */
/* import (지시자) 사용
 외부 공개 모듈 (sayHi.js) 내 기능 (sayHi 함수) 로드
 - 상대 (./sayHi.js) · 절대 (/…/sayHi.js) 경로 가능
...
- 특수 기능 필요
  - 로컬 웹서버 <sub>([`static-server`](https://www.npmjs.com/package/static-server#getting-started))</sub>
  - 라이브 서버 확장 <sub>([Live server extension](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer))</sub>
  - 기타 등등
