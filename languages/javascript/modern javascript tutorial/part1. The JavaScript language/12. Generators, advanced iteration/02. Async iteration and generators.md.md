...
||동기|비동기|
|:---:|---|---|
|메서드|`[Symbol.iterator]`|`[Symbol.asyncIterator]`|
|`next` <sub>(메서드)</sub> 반환 값|값 객체|`Promise` <sub>(객체)</sub>|
|반복문|`for (let i of it)`|`for await (let i of it)`|

##### 이터러블 비동기화 <sub>(동기 → 비동기)</sub>
1. `[Symbol.iterator]`
