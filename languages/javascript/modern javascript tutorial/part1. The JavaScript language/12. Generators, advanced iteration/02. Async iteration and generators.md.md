...
|---|---|---|
|`iterator` 반환 메서드|`[Symbol.iterator]`|`[Symbol.asyncIterator]`|
|`next()` 반환 값|`{ value:…, done: boolean }`|`Promise` <sub>(객체)</sub><br />- `{ value:…, done: boolean }`|
...
|---|---|---|
|선언|`function*`|`async function*`|
|`next()` 반환 값|`{ value:…, done: boolean }`|`{ value:…, done: boolean }` 감싸는 `Promise`|
