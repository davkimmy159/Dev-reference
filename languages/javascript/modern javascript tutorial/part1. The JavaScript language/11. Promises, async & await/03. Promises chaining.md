...
##### 3. `then` <sub>(메서드)</sub>
- 콜백 <sub>(네이티브 함수)</sub> 인수 받음 <sub>(`executor` 유사)</sub>
  - `resolve`
  - `reject`
...
fetch('/article/promise-chaining/user.json')
  ...
  /*
   .then 핸들러
   - new Promise 반환
     - setTimeout 안 resolve(githubUser) 호출 시
       - 처리 상태 변화
   다음 .then 핸들러
   - 이전 결과 대기
   */
...
##### 핸들러 <sub>(`.then` · `.catch` · `.finally`)</sub> : `Promise` 반환 시
- 나머지 체인
  - `Promise` 처리 대기
- 처리 완료 시
