...
##### 소비 함수
- 결과 · 에러 받음
- 등록 · 구독 <sub>(메서드 · 핸들러)</sub>
  - `.then`
  - `.catch`
  - `.finally`

#### `then` <sub>(메서드 · 핸들러)</sub>
- 가장 중요 · 기본
...
### `catch` <sub>(메서드 · 핸들러)</sub>

#### 에러 발생 경우만 처리 시

##### `then` <sub>(메서드 · 핸들러)</sub> 1번째 인수 → `null` 전달
- `.then(null, func)`
...
|결과 따라 다음 동작 코드 <sub>(`.then`)</sub> 작성|결과 후 동작 미리 알고 있어야 함|
