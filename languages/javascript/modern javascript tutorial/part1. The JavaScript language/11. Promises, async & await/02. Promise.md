...
##### executor <sub>(실행 함수)</sub> 처리 시작 1초 후
- `resolve("완료")` 호출
- 결과 생성
...
##### executor <sub>(실행 함수)</sub> 의해 처리 끝난 일
- 결과 · 에러만 가짐
...
##### `Promise` <sub>(객체)</sub>
- executor <sub>(실행 함수)</sub> · 소비 함수 연결
