...
async function loadJson(url) {
  ...
  if (response.status == 200) {
    ...
    /* await 대신 return response.json()
    if (response.status == 200) {
      return response.json(); // (3)
    }
    프라미스 이행
    - await 사용해 바깥 코드에서 대기 필요
     */
...
##### 프라미스 체이닝 챕터 예시 <sub>(다시 던지기)</sub> 재작성
- `.then` · `.catch`
  - → `async` · `await`
- `demoGithubUser` 내 반복
  - 반복문 사용
