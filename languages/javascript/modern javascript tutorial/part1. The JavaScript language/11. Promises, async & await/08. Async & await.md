...
async function f() {
  ...
  // (1) Promise 이행 완료 때까지 대기
  // (2) Promise 처리 후 실행 재개
...
##### `showAvatar()` <sub>(함수)</sub> 예시 <sub>(`Promise` 체이닝 챕터)</sub>
...
##### `await` : 객체 <sub>(`.then` 구현 · `Promise` X)</sub> 전달 시
...
##### `Promise` 체이닝 챕터 예시 <sub>(다시 던지기)</sub> 재작성
- `.then` · `.catch`
  - → `async` · `await`
- `demoGithubUser` 내 반복
  - 반복문 사용
