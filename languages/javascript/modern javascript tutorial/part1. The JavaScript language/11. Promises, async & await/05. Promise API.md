...
- 새 `Promise` <sub>(객체)</sub>
  ...
  - `result` <sub>(프로퍼티 · 배열)</sub> 내 결과 저장
    - `promises` <sub>(인수)</sub> 내 요소 순서 요소

##### `result` <sub>(프로퍼티 · 배열)</sub> 내 요소 <sub>(결괏값)</sub> 순서
- 인수 내 요소 <sub>(`Promise` 객체)</sub> 순서 유지
- 1번째 요소 <sub>(`Promise` 객체)</sub>
  - 가장 처음 위치 <sub>(처리 시간 무관)</sub>
...
- `result` <sub>(프로퍼티)</sub> 값 <sub>(배열)</sub>
  - `[1, 2, 3]`
```javascript
let promise = Promise.all([

  // result (프로퍼티) 내 1번째 위치 결괏값 저장
  new Promise(resolve => setTimeout(() => resolve(1), 3000)),

  // result (프로퍼티) 내 2번째 위치 결괏값 저장
  new Promise(resolve => setTimeout(() => resolve(2), 2000)),

  // result (프로퍼티) 내 3번째 위치 결괏값 저장
  new Promise(resolve => setTimeout(() => resolve(3), 1000))
]);

promise.then(alert); // 1,2,3
```
