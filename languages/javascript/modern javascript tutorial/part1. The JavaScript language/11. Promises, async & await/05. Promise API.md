...
<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **에러 발생 시 다른 프라미스 무시**
...
##### 1. `Promise.all(promises)` <sub>(가장 많이 사용)</sub>
- 모든 프라미스 이행 대기 후
  - 결괏값 배열 반환
- 하나라도 실패 시 거부
  - 나머지 프라미스 결과 무시
...
- 가장 먼저 처리된 프라미스의 결과 또는 에러를 담은 프라미스를 반환합니다.
- 가장 빠른 처리 결과 · 에러 담은 프라미스 반환
