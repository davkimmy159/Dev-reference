...
##### 배열 내 `Promise` 모두 처리 시
- 새 `Promise` 이행
- `result` <sub>(새 배열)</sub>
  - 인수 내 요소 <sub>(`Promise`)</sub> 결괏값 모음

##### 예시
- 3초 후 처리
- 반환 값 <sub>(새 `Promise`)</sub> 내 `result`
  - `[1, 2, 3]` <sub>(배열)</sub>
```javascript
/*
 모든 Promise 처리 완료 시
...
- 인수 내 요소 <sub>(`Promise`)</sub> 순서 유지
- 1번째 요소 <sub>(`Promise`)</sub>
  - 가장 늦게 이행
  - 가장 처음 위치
...
##### 전달 인수 내 요소 <sub>(`Promise`)</sub> 중 하나라도 거부 시
- 반환 값 <sub>(새 `Promise`)</sub> 상태
  - 에러 함께 바로 거부
```javascript
Promise.all([
...
<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **에러 발생 시 다른 `Promise` 무시**

##### `Promise` 하나라도 거부 시
- 즉시 거부
- 완전히 무시
  - 인수 내 다른 요소 <sub>(`Promise`)</sub> 결과
  - 이행된 `Promise` 결과
...
##### 인수
- 보통 `Promise` 요소 이러터블 <sub>(객체)</sub>
  - 대부분 배열

##### 요소
- 보통 `Promise`
- 非`Promise` <sub>(객체)</sub>
  - '그대로' 결과 배열에 전달
```javascript
Promise.all([
...
##### `.then` 핸들러 : 결괏값 변환
- `value` <sub>(`Promise` 성공 결괏값)</sub>
```javascript
{ status: 'fulfilled', value }
```
- `reason` <sub>(`Promise` 실패 결괏값)</sub>
```javascript
...
##### `loadCached` <sub>(함수)</sub> 호출 : `Promise` 반환 보장
- `loadCached(url).then(…)`
