...
##### 처리 대상 <sub>(다수 `Promise` 객체)</sub> 내 거부 발생
...
- `result` <sub>(프로퍼티 · 배열)</sub>
  - 상태 · 결괏값 저장 <sub>(`promises` 내 순서 유지)</sub>
    - 특정 객체 형태 저장 <sub>(상태 · 결괏값)</sub>
```javascript
[
  {    // 이행 (성공)
    status: "fulfilled", // 상태
