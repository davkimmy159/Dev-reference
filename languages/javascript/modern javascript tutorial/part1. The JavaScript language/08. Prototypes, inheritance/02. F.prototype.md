...
##### `constructor` <sub>(상속 프로퍼티)</sub> 활용
- 새 객체 생성 시 기존 객체 내 프로퍼티 사용
  - 생성자 확인 불가능 시 유용
    - ex\) 서드파티 라이브러리 객체 등
```javascript
function Rabbit(name) {
...
##### `constructor` <sub>(프로퍼티)</sub> 중요 특징
- 적절한 값 보장 X <sub>(JS 언어 차원)</sub>
- 함수 내 `prototype` <sub>(프로퍼티)</sub> 기본값 설정
  - JS 담당
- `constructor` <sub>(프로퍼티)</sub> 관련 모든 동작
  - 개발자 담당
