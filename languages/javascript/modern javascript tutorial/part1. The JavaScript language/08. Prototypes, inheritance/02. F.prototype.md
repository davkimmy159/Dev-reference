...
##### 과거 프로토타입 직접 접근 방법 X
- 그나마 믿고 사용할 수 있었던 방법
  - 생성자 <sub>(함수)</sub> `prototype` <sub>(프로퍼티)</sub>
- 많은 스크립트
  - 아직 이 방법 사용
...
##### 생성자 <sub>(함수)</sub> 디폴트 프로퍼티 <sub>(`F.prototype`)</sub>
- ≠ `[[Prototype]]` <sub>(숨김 프로퍼티)</sub>
- 새 객체 <sub>(`new F()`)</sub> `[[Prototype]]` 설정
- 객체 · `null` 할당 가능
  - 다른 값 무시
- 생성자 <sub>(함수)</sub> · `new` 함께 호출 때만 적용
...
### 동일한 생성자 <sub>(함수)</sub> 로 객체 만들기

##### 생성자 <sub>(함수)</sub> 사용해 새 객체 생성 상황
- 생성자 <sub>(함수)</sub> 1개 존재
- 임의 객체 <sub>(`obj`)</sub> 1개 존재
  - 생성자 <sub>(함수)</sub> 사용해 생성
