...
함수는 내장 객체 `Function`의 생성자를 사용해 만들어지는데 `call`, `apply`를 비롯한 함수에서 사용할 수 있는 메서드는 `Fuction.prototype`에서 받아옵니다. 참고로 함수에도 `toString`이 구현되어있습니다.

##### `Function` <sub>(내장 객체)</sub> 생성자
- 생성 함수 내 `[[Prototype]]` 설정
  - `Function.prototype` 참조
    - 함수 메서드 상속 <sub>(`call` · `apply` 등)</sub>
```javascript
function f() {}
