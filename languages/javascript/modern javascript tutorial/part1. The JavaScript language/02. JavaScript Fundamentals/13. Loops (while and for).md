...
##### `i < 3` <sub>(조건)</sub> 만족 시 계속 `i` <sub>(변수)</sub> 출력
```javascript
let i = 0;
...
##### `i` → `0` <sub>(숫자)</sub> 도달 시 <sub>(falsy)</sub> 반복 종료
```javascript
let i = 3;
...
- `true` 평가 시 <sub>(truthy)</sub> 반복문 본문 계속 실행
- 본문 최소 1번 실행
