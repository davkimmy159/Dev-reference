...
##### `==` <sub>(동등 비교)</sub> vs `Boolean(…)` <sub>(논리 평가)</sub>
- `==` <sub>(동등 비교 연산자)</sub>
  - 피연산자 자동 형 변환 <sub>(`number`)</sub>
- `Boolean(…)` <sub>(`boolean` 생성자)</sub>
  - 인수 명시적 형 변환 <sub>(`boolean`)</sub>
```javascript
let a = 0;
...
- 타 자료형 비교 시
  - `number` 형 자동 변환
- `0` <sub>(숫자)</sub> · `"0"` <sub>(문자열)</sub> · `""` <sub>(빈 문자열)</sub> · `false` 구별 불가능
  - `0` <sub>(숫자)</sub> 자동 형 변환
