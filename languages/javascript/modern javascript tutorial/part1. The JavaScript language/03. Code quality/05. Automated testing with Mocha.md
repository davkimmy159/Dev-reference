...
### 중첩 `describe`
- `makeTest` <sub>(헬퍼 함수)</sub> · `for` <sub>(반복문)</sub>
  - 중첩 `describe` 내 함께 묶여있음

는 것을 눈여겨보시기 바랍니다. `makeTest`는 오직 `for`문에서만 사용되고, 다른 데선 사용되지 않기 때문에 이렇게 묶어놓았습니다. 아래 스펙에서 `makeTest`와 `for`문은 함께 어우러져 `pow`가 제대로 동작하는지 확인해주는 역할을 합니다.
