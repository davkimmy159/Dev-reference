...
- target
  - the object being registered for tracking.
  - If the target is garbage collected, the cleanup callback will be called with heldValue as its argument.
...
- 메모리 내 제거 시
  - 청소 콜백 자동 호출
    - `heldValue` 전달
- unregisterToken
  - an unregistration token. It can be passed to unregister an object before the garbage collector deletes it. Typically, the target object is used as unregisterToken, which is the standard practice.

##### `[unregisterToken]`
- 제거 토큰
- 등록 객체 메모리 내 제거 전 제거 시 사용
- 등록 객체 제거 시 사용
  - 가비지 컬렉터 의해 삭제 전
