...
- 주기적으로 캐시 정리
  - 참조 사라진 해당 키 · 값 정리
- 청소 콜백 <sub>(함수)</sub> <sub>(finalizer)</sub> 사용
...
- `target`
  ...
  the object being registered for tracking. If the target is garbage collected, the cleanup callback will be called with heldValue as its argument.
