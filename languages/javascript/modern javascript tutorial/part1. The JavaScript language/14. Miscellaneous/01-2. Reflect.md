...
|---|---|---|---|
|`obj[prop]`|읽기|`Reflect.get(obj, prop)`|`[[Get]]`|
|`obj[prop] = value`|쓰기|`Reflect.set(obj, prop, value)`|`[[Set]]`|
|`delete obj[prop]`|삭제|`Reflect.deleteProperty(obj, prop)`|`[[Delete]]`|
|`new F(value)`|객체 생성|`Reflect.construct(F, value)`|`[[Construct]]`|
|…|…|…|…|
...
array[-1]; // 3, 뒤에서 1번째 (마지막) 요소
array[-2]; // 2, 뒤에서 2번째 요소
array[-3]; // 1, 뒤에서 3번째 요소
```

##### 음수 인덱스 사용해 배열 요소 접근 프락시 작성
```javascript
let array = [1, 2, 3];
...
- `observe` <sub>(메서드)</sub> 구현
  - `handler` <sub>(함수)</sub> 설정 기능
    - 프로퍼티 변경 시 호출
- `handler(key, value)` <sub>(메서드)</sub>
  - 프로퍼티 변경 때마다 호출
  - 인수
    - 프로퍼티 키 · 값
