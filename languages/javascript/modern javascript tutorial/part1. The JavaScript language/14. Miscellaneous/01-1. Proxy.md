...
##### 순환 동작별 차이점
- `Object.getOwnPropertyNames` <sub>(반환)</sub>
  - `symbol` 외 키
- `Object.getOwnPropertySymbols` <sub>(반환)</sub>
  - `symbol` 키
- `Object.[key·value]s` <sub>(반환)</sub>
  - `enumerable: true` · `symbol` 외 키 · 값
  - `symbol` 외 키 · 값
- `for…in` <sub>(순회)</sub>
  - `enumerable: true`
  - `symbol` 외 키
