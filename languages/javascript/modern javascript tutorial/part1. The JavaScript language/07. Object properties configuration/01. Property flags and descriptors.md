...
<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **엄격 모드 vs 非엄격 모드**

|모드|엄격|非엄격|
|:---:|:---:|:---:|
|플래그 규칙 위반|에러|에러 X <sub>(무시)</sub>|
|읽기 전용 프로퍼티 수정|에러|에러 X <sub>(무시)</sub>|

##### 非엄격 모드
- 플래그 규칙 위반 시 에러 X <sub>(무시)</sub>
- 읽기 전용 프로퍼티 값 수정 시 에러 X <sub>(값 변경 X)</sub>
