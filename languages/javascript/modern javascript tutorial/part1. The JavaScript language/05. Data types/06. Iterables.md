...
### iterator <sub>(객체)</sub> 명시적 호출 <sub>(드물게 발생)</sub>
...
##### 이터러블 <sub>(객체)</sub> · 유사 배열 객체 → 배열 변환
- 배열 메서드 사용 가능
- 이터러블 <sub>(객체)</sub> · 유사 배열 객체 여부 검사
...
// 각 숫자 제곱 연산 후 저장
let arr = Array.from(arrayLike, num => num * num);

alert(arr); // 1,4,9,16,25
```

##### 문자열 → 배열
- 문자열 자체 이터러블 <sub>(객체)</sub> 속성 이용 <sub>(`Array.from`)</sub>
  - 해 동작합니다. 따라서 for..of처럼 서로게이트 쌍에도 제대로 적용됩니다.
