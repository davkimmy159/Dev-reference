...
##### 모던 JS 엔진
- 'hot code' <sub>(다수 실행 코드)</sub> 대상 최적화 수행
...
##### `Date` <sub>(객체)</sub> → `number` 형 변환 시 타임스탬프 반환
- `Date` <sub>(객체)</sub> 간 차감 가능
  - 시차 <sub>(ms)</sub> 계산
...
- 대다수 호스트 환경 <sub>(브라우저 등)</sub> 지원
  - [`performance.now()`](https://developer.mozilla.org/en-US/docs/Web/API/Performance/now) <sub>(브라우저 메서드)</sub>
