...
alert( 'diffSubtract 십만번 호출 소모 시간: ' + subResult + 'ms' );
alert( 'diffGetTime  십만번 호출 소모 시간: ' + getResult + 'ms' );
```
...
- 벤치마크 중 타 작업 병렬 처리 <sub>(CPU 자원 투입)</sub> 가능성 有
- 벤치마크 영향 (부정확한 결과)
