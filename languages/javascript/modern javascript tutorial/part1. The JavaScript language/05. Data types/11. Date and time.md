...
- `date` <sub>(`Date` 객체)</sub> · 현재 일시 비교
  - 특정 조건 만족 시 특정 문자열 반환

|`date` 비교 <sub>(현재 일시 기준)</sub>|반환 값|
|---|---|
|1초 미만 전|`"현재"`|
|1분 이하 전|`"n초 전"`|
|1시간 미만 전|`"n분 전"`|
...
let dNow = Date.now();
let now = dNow - 1
let s30 = dNow - 30 * 1000;
let m5  = dNow - 5  * 60 * 1000;
let ytd = dNow - 24 * 60 * 60 * 1000;

formatDate(new Date(now)); // 현재
