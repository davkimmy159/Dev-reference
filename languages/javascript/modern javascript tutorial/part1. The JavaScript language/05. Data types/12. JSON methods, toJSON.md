...
##### `[replacer]` <sub>(`JSON.stringify` 메서드 2번째 인수)</sub>
- 변환 프로세스 세밀 조정
  - ex\) 순환 참조 등
...
##### `[space]` <sub>(`JSON.stringify` 메서드 3번째 인수)</sub>
- 중간 삽입 공백 문자 수
```javascript
let user = {
  name: "John",
...
##### `[reviver]` <sub>(`JSON.parse` 메서드 2번째 인수)</sub>
- 변환 프로세스 세밀 조정 
  - ex\) 인코딩 객체 → 기존 타입 객체
