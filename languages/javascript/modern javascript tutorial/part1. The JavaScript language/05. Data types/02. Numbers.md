...
- 64비트 형식 [IEEE-754](https://en.wikipedia.org/wiki/IEEE_754-2008_revision)
...
- 최근접 값 올림 · 버림 <sub>(`Math.round` 유사)</sub>
```javascript
let num1 = 12.34;
...
##### 숫자 저장 시 정확히 64비트 필요
- 내부적 64비트 형식 [IEEE-754](https://en.wikipedia.org/wiki/IEEE_754-2008_revision) 표현

되기 때문에 숫자를 저장하려면 정확히 64비트가 필요합니다. 64비트 중 52비트는 숫자를 저장하는 데 사용되고, 11비트는 소수점 위치를(정수는 0), 1비트는 부호를 저장하는 데 사용됩니다.
