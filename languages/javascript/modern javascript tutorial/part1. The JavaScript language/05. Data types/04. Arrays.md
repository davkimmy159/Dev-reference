...
fruits.shift(); // "Apple" (1번째 요소) 반환
```

![array-shift](../../images/01/05/04/array-shift.svg)

##### `[0]` <sub>(1번째 요소)</sub> 대상 작업 <sub>(추가 · 제거)</sub>
- 이후 나머지 요소 수정 필요

##### 과정
1. `[0]` <sub>(1번째 요소)</sub> 요소 제거 · 추가
2. 전체 요소 좌 · 우 이동 <sub>(인덱스 1씩 감소 · 증가)</sub>
   - `[1]` ↔ `[0]`
   - `[2]` ↔ `[1]`
   - 기타 등등
