...
// {name: "John"} (객체) == 배열 요소 (array[0])
// - 가비지 컬렉션 대상 X
alert(JSON.stringify(array[0]));
```

##### `Map` <sub>(객체)</sub> 객체형 키
- 메모리 내 해당 `Map` <sub>(객체)</sub> 유지 시
  - 객체형 키 유지 <sub>(가비지 컬렉션 대상 X)</sub>
```javascript
let john = {name: "John"};
let map  = new Map();
...
- 가비지 컬렉션 대상 <sub>(도달 불가능 상태 취급)</sub>
  - `WeakMap` <sub>(객체)</sub> · 메모리 내 자동 제거
