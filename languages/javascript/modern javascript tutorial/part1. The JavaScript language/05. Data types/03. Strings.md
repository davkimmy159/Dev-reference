...
||`\r\n` <sub>(캐리지 리턴 + 줄 바꿈)</sub><br />- Windows 줄 바꿈|
|`\'` · `\"`|따옴표|
|`\\`|역슬래시|
|`\t`|탭|
|`\b`|백스페이스 <sub>(호환성 유지 · 최근 미사용)</sub>|
|`\f`|폼 피드 <sub>(호환성 유지 · 최근 미사용)</sub>|
|`\v`|세로 탭 <sub>(호환성 유지 · 최근 미사용)</sub>|
|`\b` · `\f` · `\v`|백스페이스 ·폼 피드 · 세로 탭|
||호환성 유지 목적 <sub>(최근 미사용)</sub>|
|`\x□□`|유니코드 글자 <sub>(16진수)</sub>|
||ex\) `\x7A` → `z` <sub>(알파벳)</sub>|
|`\u□□□□`|유니코드 기호 <sub>(16진수)</sub>|
||ex\) `\u00A9` → `©` <sub>(저작권 기호)</sub>|
