...
##### `String.includes(substr, [pos])` <sub>(메서드)</sub>
...
#### `String.slice(start [, end])` <sub>(메서드)</sub>
...
#### `String.substring(start [, end])` <sub>(메서드)</sub>
...
#### `String.substr(start [, length])` <sub>(메서드)</sub>
...
#### `String.codePointAt(pos)` <sub>(메서드)</sub>
- `pos` 위치 글자 코드 반환
```javascript
...
#### `String.fromCodePoint(code)` <sub>(메서드)</sub>
- 숫자 형식 `code` 대응 글자 생성
- `\u` 뒤 특정 코드 붙이는 방식 가능
  - 글자 대응 16진수 코드
```javascript
