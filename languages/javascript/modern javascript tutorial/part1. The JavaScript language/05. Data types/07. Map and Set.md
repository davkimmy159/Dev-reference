...
주요 메서드와 프로퍼티:
- `new Map([iterable])` – 맵을 만듭니다. `[key,value]`쌍이 있는 `iterable`(예: 배열)을 선택적으로 넘길 수 있는데, 이때 넘긴 이터러블 <sub>(객체)</sub>는 맵 초기화에 사용됩니다.
...
주요 메서드와 프로퍼티:
- `new Set([iterable])` – 셋을 만듭니다. `iterable` 객체를 선택적으로 전달받을 수 있는데(대개 배열을 전달받음), 이터러블 <sub>(객체)</sub> 안의 요소는 셋을 초기화하는데 쓰입니다.
...
### 애너그램 걸러내기
[애너그램(어구전철)](https://ko.wikipedia.org/wiki/%EC%96%B4%EA%B5%AC%EC%A0%84%EC%B2%A0)은 단어나 문장을 구성하고 있는 문자의 순서를 바꾸어 다른 단어나 문장을 만드는 놀이입니다.
