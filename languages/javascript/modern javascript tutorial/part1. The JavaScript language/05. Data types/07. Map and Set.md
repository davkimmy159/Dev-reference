...
|메서드 · 프로퍼티|기능|
|---|---|
|`new Map([iterable])`|`Map` <sub>(객체)</sub> 생성|
||초기화 인수<br />- 이터러블 <sub>(키 · 값 쌍 구성 객체)</sub>|
|`set(key, value)`|`key` · `value` 쌍 저장 후 자신 반환|
...
##### `new Map([iterable])` <sub>(생성자)</sub>
- `Map` <sub>(객체)</sub> 생성
- `[iterable]` <sub>(초기화 인수)</sub>
  - 이터러블 <sub>(키 · 값 쌍 구성 객체)</sub>
