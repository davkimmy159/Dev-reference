...
alert( slow(1) ); // 캐시 내 slow(1) 호출 호출 저장
alert( slow(1) ); // 캐시 내 결괏값 반환

alert( slow(2) ); // 캐시 내 slow(2) 호출 결과 저장
alert( slow(2) ); // 캐시 내 결괏값 반환
```

![decorator-makecaching-wrapper](../../images/01/06/09/decorator-makecaching-wrapper.svg)

##### `cachingDecorator(func)` <sub>(함수)</sub>
- 모든 함수 전달 가능
- `function(x)` <sub>(래퍼 함수)</sub> 반환

##### `function(x)` <sub>(래퍼 함수)</sub>
- `cachingDecorator(func)` <sub>(함수)</sub> 반환 값
- `func(x)` <sub>(기존 함수)</sub> 호출 결과
  - 캐싱 로직으로 wrapping

##### 적용 과정
1. `cachingDecorator(func)` <sub>(함수)</sub> 호출
   - `function(x)` <sub>(래퍼 함수)</sub> 반환
2. `function(x)` <sub>(래퍼 함수)</sub> 호출
