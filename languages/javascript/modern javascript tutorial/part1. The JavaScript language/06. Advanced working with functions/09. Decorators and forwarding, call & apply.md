...
기존에 `call`을 사용했던 방식처럼 `this`를 받고 `this[0], this[1]` 등이 합쳐집니다. 이렇게 내부 알고리즘이 구현되어있기 때문에 어떤 유사 배열이든 `this`가 될 수 있습니다. 상당수의 메서드가 이런 관습을 따르고 있죠. `this`에 `arguments`가 할당되더라도 잘 동작하는 이유가 여기에 있습니다.
