...
1. 일반 배열 내 `join` <sub>(메서드)</sub> 빌리기
   - `[].join`
2. 컨텍스트 <sub>(`arguments`)</sub> 고정
   - `[].join.call`를 사용해 arguments를 컨텍스트로 고정한 후 join메서드를 호출하는 것이죠.

1. `[]` <sub>(빈 배열)</sub> 대상 `join` <sub>(메서드)</sub> 호출
2. 
```javascript
function hash() {
