...
##### 용도
- 보통 객체 메서드 `this` 고정 후 전달
  - ex&#41; `setTimeout` <sub>(함수)</sub> 전달
...
##### [묶인 함수 <sub>(bound function)</sub>](https://tc39.es/ecma262/#sec-bound-function-exotic-objects)
- `f.bind( … )` 반환 특수 객체
- 컨텍스트 · 인수 고정
...
- 복잡한 상황 <sub>(중간에 `user` 변수 변경 등)</sub> 발생 시
  - 오작동 가능성 有
    - `askPassword` 호출됐으나 사용자가 프롬프트 대화상자에 값을 제출하고 `() => user.loginOk()`를 호출하기 전에 `user` 변수가 바뀌는 등의 복잡한 상황에서는 오작동할 가능성이 있습니다.
