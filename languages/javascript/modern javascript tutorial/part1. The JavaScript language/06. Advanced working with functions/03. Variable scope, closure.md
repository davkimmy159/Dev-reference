...
- 외부 <sub>(전역)</sub> 렉시컬 환경 참조 값 저장

##### 외부 <sub>(전역)</sub> 렉시컬 환경
- 프로퍼티 2개 저장
  - `phrase` <sub>(변수)</sub>
  - `say` <sub>(함수)</sub>
...
#### `say(name)` <sub>(함수)</sub> 내 변수 검색

##### `name` <sub>(변수)</sub>
1. 내부 렉시컬 환경 검색
   - `name` <sub>(상응 프로퍼티)</sub> 有
     - 검색 성공

##### `phrase` <sub>(변수)</sub>
1. 내부 렉시컬 환경 검색
   - `phrase` <sub>(상응 프로퍼티)</sub> 無
     - 검색 실패
2. 외부 렉시컬 환경 검색 <sub>(참조 이용)</sub>
   - `phrase` <sub>(상응 프로퍼티)</sub> 有
     - 검색 성공
