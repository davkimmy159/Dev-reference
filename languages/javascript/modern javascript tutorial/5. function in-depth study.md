...
```
- `for…of` 같은 방식
  - 내부에서 이터레이터 (iterator, 반복자) 사용해 요소 수집
- 문자열 `for…of` 사용
  - 문자열 구성 문자 반환
- `...str`
  - `H,e,l,l,o`
    - 배열 초기자 (array initializer) `[...str]` 로 전달
- `Array.from`
  - 이터러블 객체 문자열 → 배열 변환
  - 동일한 작업 수행
```javascript
...
1. 스크립트 시작 시 스크립트 내 선언 변수 전역 렉시컬 환경 올라감 (pre-populated)
    - 변수 상태
      - uninitialized (특수 내부 상태)
      - 자바스크립트 엔진 uninitialized 상태 변수 인지
        - let 만나기 전지 변수 참조 X
2. let `phrase` 등장 (값 할당 전)
    - 값 : `undefined`
...
- `counter.[[Environment]]`
  ...
  - 호출 장소 상관없이 함수 자신이 태어난 곳 기억 (`[[Environment]]` 프로퍼티 덕분)
...
let arr = [f(), f(), f()];
```
- 렉시컬 환경 객체 (다른 객체와 마찬가지로) 도달할 수 없을 때 메모리에서 삭제
...
```
- 폴리필 추가
  - 오래된 브라우저 (명세 존재 기능 미지원) 사용 시 직접 함수 생성 후 전역 객체에 추가
```javascript
if (!window.Promise) {
...
- 함수
  - 호출 가능한 (callable) '행동 객체'
    - 호출 가능
    - (객체 같이) 프로퍼티 추가 · 제거 · 참조 통해 전달
...
```
- 나머지 매개변수 개수 포함 X
- 다른 함수 안 동작 함수 타입 검사 (type introspection) 종종 사용
```javascript
...
- 외부 변수 접근 시
  ...
  - 프로덕션 서버 반영 전 압축기 (minifier) 의해 압축될 때
    - 압축기 지역 변수 이름 짧게 바꾸면서 문제 발생
    - new Function 문법 생성 함수 내부에서 외부 (외부 렉시컬 환경) 변수 접근 시 기존 변수명 이미 변경
      - 찾을 수 없음
    - 함수 내부에서 외부 변수 접근
      - 아키텍처 관점 권장 X
      - 에러 취약
...
- `setTimeout` 호출 시 '타이머 식별자(timer identifier)' 반환
  - 스케줄링 취소 시 식별자 사용
```javascript
...
- `cachingDecorator(func)` 호출 시 래퍼 (wrapper) `function(x)` 반환
...
- `setTimeout` 화살표 함수
  - 간결성 · 취약성 ↑
  - `setTimeout` 트리거 되기 전 (1초 지나기 전) `user` 변경 시 변경된 객체 메서드 호출
```javascript
...
```
- 가독성 좋은 이름 (double, triple) 가진 독립 함수 생성 가능
- 1번째 인수 고정
  - 매번 인수 전달 필요 X
