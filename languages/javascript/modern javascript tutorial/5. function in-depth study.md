...
```
- `x * pow (x, n - 1)` 계산하려면
  - 새 인수 들어가는 `pow` 서브 호출 <sub>(subcall)</sub>, `pow (2, 2)` 만들어야 함

#### `pow(2, 2)`
- 중첩 호출 위해 자바스크립트 실행 컨텍스트 스택에 현재 실행 컨텍스트 저장
- 프로세스
  1. 스택 최상단 현재 컨텍스트 '기록'
  2. 서브 호출 위한 새 컨텍스트 생성
...
#### pow(2, 1)
- 동일한 과정 다시 반복
- 5번째 줄 인수 `x = 2, n = 1` 함께 새 서브 호출 생성
- 새 실행 컨텍스트 생성 후 이전 실행 컨텍스트 스택 최상단 push
```
...
- 배열 : 요소 삭제 · 삽입 비용 ↑
  - `arr.unshift(obj)`
    - 새 obj 위한 공간 생성 위해 모든 요소 번호 다시 매김
    - 배열 크기 ↑ → 연산 수행 시간 ↑
  - `arr.shift()`
    - 동일
  - `arr.push · pop`
...
- 코드에서 변수 접근 시
  ...
  - 전역 렉시컬 환경 도달 때까지 변수 찾지 못하면 엄격 모드 시 에러 발생
    - 비 엄격 모드 시 미정의 변수에 값 할당 시 에러 발생 대신 새 전역 변수 생성
      - 하위 호환성 위해 남은 기능
- 예시 정리
...
```
- `makeCounter()` 호출 시
  - 호출 때마다 새 렉시컬 환경 객체 생성 후 `makeCounter` 실행에 필요한 변수들 저장
  - 두 개의 렉시컬 환경 생성
