...
// new Rabbit 호출해 만든 새 객체 [[Prototype]] → animal 설정
...
- 새 객체 생성 후 `F.prototype` 프로퍼티 바뀌면 <sub>(F.prototype = &#60;another object&#62;)</sub>
  - `new F` 호출해 생성한 또 다른 새 객체 `another object` `[[Prototype]]`로 보유
...
- `constructor` 프로퍼티
  - 기존 객체 `constructor` 사용해 새 객체 만들때 사용
```javascript
function Rabbit(name) {
  this.name = name;
  alert(name);
