...
<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`connectedCallback` 내 랜더링 <sub>(생성자 X)</sub>**
...
##### 1. `HTMLButtonElement` <sub>(버튼 클래스)</sub> 상속
```javascript
...
##### `<live-timer>` <sub>(요소)</sub> 작성
1. `<time-formatted>` <sub>(요소 · 튜토리얼)</sub>
    - 내부적 사용
    - 기능 중복 X
2. 매초 tick 갱신
3. `event.detail` <sub>(모든 tick 대상)</sub>
    - 현재 일시
    - `tick` <sub>(커스텀 이벤트)</sub> 생성
```html
...
##### 1. 요소 내 문서 제거 시 타이머 <sub>(`setInterval`)</sub> 제거 필요
- 미제거 시
  1. 계속 tick
  2. 메모리 낭비

##### 2. `elem.date` <sub>(프로퍼티)</sub>
- 현재 일시 접근
- 모든 클래스 메서드 · 프로퍼티
  - → 요소 메서드 · 프로퍼티
