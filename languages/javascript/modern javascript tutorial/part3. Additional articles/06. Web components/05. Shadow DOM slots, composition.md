...
##### 기명 `slot`
- `<slot name="X">…</slot>` <sub>(Shadow DOM)</sub>
  - `<… slot="X">` <sub>(Light DOM)</sub>

##### 기본 `slot`
- the first `<slot>` without a name (subsequent unnamed slots are ignored)
- gets unslotted light children.

##### If there are many elements for the same slot
- they are appended one after another.
