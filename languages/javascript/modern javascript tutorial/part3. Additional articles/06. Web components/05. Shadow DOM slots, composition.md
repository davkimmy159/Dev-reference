...
- 내용 기본값 역할
  - 상응 `slot` <sub>(속성)</sub> 요소 부재 시 렌더링
...
- 기본 <sub>(1번째 무기명)</sub> `<slot>` <sub>(요소)</sub>
  ...
  - `<div>` <sub>(요소 · Light DOM)</sub> 2개 렌더링
    - `slot` <sub>(속성)</sub> X
```html
<script>
customElements.define('user-card', class extends HTMLElement {
...
- 복사 X · 렌더링 <sub>(상응 위치)</sub>
  - 변경 사항 바로 렌더링

#### `slotchange` <sub>(이벤트)</sub>

##### `<slot>` <sub>(요소)</sub> 내부 렌더링 요소 변화 시 발생
- 추가 · 제거 · 대체
```html
<ul>
  <slot name="item">
...
##### 1. 초기화 과정 <sub>(`slot` 매칭 · 내부 렌더링)</sub>
- `<slot name="item">` <sub>(요소 · Shadow DOM)</sub>
...
##### `{ mode: 'open' }` <sub>(옵션 · Shadow 트리)</sub> 설정 시
...
- `slotchange` event
  - triggers the first time a slot is filled, and on any add/remove/replace operation of the slotted element, but not its children. The slot is `event.target`.
