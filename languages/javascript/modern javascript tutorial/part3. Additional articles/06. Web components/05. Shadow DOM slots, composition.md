...
##### 다양한 컴포넌트 <sub>(탭 · 메뉴 · 이미지 갤러리 등)</sub>
- 내용 렌더링 필요
- ex\) `<select>` <sub>(브라우저 내장 요소)</sub>
  - expects `<option>` items, our `<custom-tabs>` may expect the actual tab content to be passed. And a `<custom-menu>` may expect menu items.
