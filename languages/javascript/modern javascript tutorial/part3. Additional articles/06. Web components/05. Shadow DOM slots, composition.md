...
##### Shadow DOM 내 `<slot>` 삽입 시 평탄화 DOM 파생
- 브라우저
  - 렌더링 대상
  - 스타일 상속 · 이벤트 전파 시 사용
- JS
...
<img src="../../images/commons/icons/triangle-exclamation-solid.svg" /> **최상위 자식 요소들만 `slot` <sub>(속성)</sub> 보유 가능**

##### `slot="…"` <sub>(속성)</sub> 명시 가능 요소
- Shadow host <sub>(`<user-card>` 요소)</sub> 바로 아래 자식 요소들
