...
##### `node.assignedSlot`
- `node` 할당된 `<slot>` <sub>(요소)</sub> 반환

##### `slot.assignedNodes({ flatten: boolean })`
- `slot` 내 DOM 노드 반환
- `{ flatten: boolean }` <sub>(평탄화 DOM 깊은 검색 여부)</sub>
  - 중첩 `<slot>` <sub>(중첩 컴포넌트)</sub>
  - `<slot>` <sub>(요소)</sub> 내용 기본값
- `{ flatten: false }` <sub>(기본값)</sub>
  - 평탄화 DOM 내부 얕은 검색
...
##### `slot.assignedElements({ flatten: boolean })`
