...
- `.dark-theme` <sub>(클래스)</sub> 위치
  - 커스텀 요소 자체
  - 조상 요소
```html
<!-- 조상 요소 내 .dark-theme (클래스) 有 -->
...
1. `<slot>` <sub>(요소)</sub> 자체 스타일 <sub>(CSS 상속)</sub>
2. `::slotted(…)` <sub>(가상 클래스)</sub>
...
##### 한계
- 특정 CSS 프로퍼티 상속 불가능 <sub>(CSS 한계)</sub>
...
customElements.define('user-card', class extends HTMLElement {
  connectedCallback() {
    this.attachShadow({ mode: 'open' });
    let content = document.getElementById('tmpl').content.cloneNode(true);
...
##### 컨포넌트 <sub>(Shadow DOM)</sub> 스타일 적용
- Shadow 트리
- _"Shadow tree host"_ <sub>(커스텀 요소 자체)</sub>
  - `:host…` <sub>(가상 클래스)</sub>
- `slot` 내 렌더링 요소 <sub>(Light DOM)</sub>
  - `::slotted(…)` <sub>(자식 노드 X)</sub>

##### 문서 <sub>(Light DOM)</sub> 스타일 적용
- _"Shadow tree host"_ <sub>(커스텀 요소 자체)</sub>
