...
##### `:host` <sub>(`<custom-dialog>`)</sub>
- 중앙 정렬 <sub>(요소 자체 스타일 적용)</sub>
```html
<template id="tmpl">
  <style>
    /* 커스텀 요소 자제 스타일 */
...
##### `:host([centered])` <sub>(`<custom-dialog centered>`)</sub>
- 중앙 정렬 <sub>(요소 자체 스타일 적용)</sub>
```html
<template id="tmpl">
  <style>
...
##### `:host-context(.dark-theme)`
 matches only if there's `dark-theme` class on `<custom-dialog>` on anywhere above it:
```html
