...
##### `advance(count)`
- `count` 번 `cursor` 이동 <sub>(결과 생략)</sub>
...
##### [`jakearchibald`](https://github.com/jakearchibald/idb) <sub>(가벼운 `Promise` 객체 래퍼 라이브러리)</sub>
...
##### `Promise` <sub>(객체)</sub> 래퍼 · `async` · `await`
- 동일 결과
