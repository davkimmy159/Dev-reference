...
##### 다양한 추가 처리 동작
- 연결 종료 지연 <sub>(일정 시간)</sub>
  - 방문자측 정보 저장 요청
- `onversionchange` <sub>(핸들러)</sub>
- 구식 DB 연결 미종료 <sub>(`onversionchange` 핸들러)</sub>

- The new updating connection will be blocked immediately after `db.onversionchange` has finished without closing, and we can ask the visitor in the new tab to close other tabs for the update.
