...
1. 정확한 키
2. 범위 질의
    - [`IDBKeyRange`](https://www.w3.org/TR/IndexedDB/#keyrange) <sub>(객체)</sub> 사용
      - 키 범위 표현
...
- `lower` 이상
- `lower` 초과 <sub>(`open` → `true` 전달)</sub>

##### `upperBound(upper, [open])`
- `upper` 이하
- `upper` 미만 <sub>(`open` → `true` 전달)</sub>

##### `bound(lower, upper, [lowerOpen], [upperOpen])`
- between `lower` and `upper`. If the open flags is true, the corresponding key is not included in the range.
