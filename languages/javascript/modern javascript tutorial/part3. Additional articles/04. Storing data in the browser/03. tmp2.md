...
##### `keyPath` <sub>(문자열)</sub>
- 
- path to the object field that the index should track (we're going to search by that field),

##### `option` <sub>(선택 · 객체)</sub> – an optional object with properties:
- `unique`
  - if true, then there may be only one object in the store with the given value at the `keyPath`. The index will enforce that by generating an error if we try to add a duplicate.
- `multiEntry`
  - only used if the value on `keyPath` is an array. In that case, by default, the index will treat the whole array as the key. But if `multiEntry` is true, then the index will keep a list of store objects for each value in that array. So array members become index keys.
