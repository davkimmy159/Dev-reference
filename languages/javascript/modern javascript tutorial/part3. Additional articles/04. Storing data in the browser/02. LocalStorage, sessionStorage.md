...
- `getItem(key)`
  - 키 <sub>(`key`)</sub> 해당 값 얻기
- `removeItem(key)`
  - 키 <sub>(`key`)</sub> 해당 값 삭제
- `clear()`
  - 전체 삭제
- `key(index)`
  - 인덱스 <sub>(`index`)</sub> 해당 키 얻기
- `length`
  - 저장 항목 개수

##### `Map` <sub>(내장 객체)</sub> 유사 <sub>(차이 有)</sub>
...
#### `localStorage` <sub>(객체)</sub> 키 · 값 타입
...
##### `localStorage` <sub>(객체)</sub> 비해 덜 사용
- 프로퍼티 · 메서드 동일
- 훨씬 제한적인 사용법
...
##### `localStorage` <sub>(객체)</sub> 기반 폴리필
- 브라우저 종류 무관
...
|---|---|---|---|
|`localStorage` <sub>(객체)</sub>|창 · 탭|브라우저 종료 후 재시작||
|`sessionStorage` <sub>(객체)</sub>|탭 · `iframe`|페이지 새로 고침|브라우저 · 탭 종료|
