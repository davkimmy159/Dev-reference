...
<script>
  iframe.onload = function() {
    ...
    // 핸들러 제거
    // - window.location (주소 프로퍼티) 값 수정 (페이지 이동) 후 실행 X
    iframe.onload = null;
  };
</script>
```
...
- `iframe` 내 `window` <sub>(창 객체)</sub> 컬렉션
...
- `parent`
  - 부모 <sub>(바깥)</sub> `window` <sub>(창 객체)</sub> 참조
- `top`
...
##### 현재 `document` 위치 확인 <sub>(`iframe` 내부 여부)</sub>
- `top` <sub>(프로퍼티)</sub> 체크
```javascript
if (window == top) {
...
- 빈 속성값 <sub>(기본값 · 가장 엄격한 제한사항 적용)</sub>
  - 상이 오리진 정책 <sub>(상이 오리진 취급)</sub>
    - `src` <sub>(속성)</sub> 내 동일 오리진 무시
    - 암시적인 내부 스크립트 제한
  - 기타 등등
- 제한 완화
  - 공백 구분 리스트 <sub>(문자열)</sub> 할당
```html
...
- 새 팝업창 `window` <sub>(창 객체)</sub> 생성 · 참조 반환
...
- 메시지 전송 <sub>(`postMessage` 메서드)</sub>
...
- `allow-same-origin` <sub>(속성값)</sub>
  - 상이 오리진 상태 무효 <sub>(동일 오리진 취급)</sub>
...
##### 1. 전송 창 내 전달 대상 창 `postMessage` <sub>(메서드)</sub> 호출
```javascript
