...
- 전달 대상 창 오리진 <sub>(안전 보장 목적)</sub>
  - 대상 창 오리진 제한
- 대상 창 <sub>(`window`)</sub> 객체 <sub>(타 오리진)</sub>
...
##### `sandbox` <sub>(`iframe` 속성)</sub> 
- 상이 오리진 상태 강제
  - `allow-same-origin` <sub>(속성)</sub> 값 할당 시 무효
- unless the `allow-same-origin` is specified in the attribute value. That can be used to run untrusted code in iframes from the same site.
