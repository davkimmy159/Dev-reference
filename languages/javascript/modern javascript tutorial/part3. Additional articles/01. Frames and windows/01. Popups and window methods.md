...
- 새 창 이름
- 각 창 내 해당 프로퍼티 有
  - 팝업창 내 내용 표시할 창 선택 가능
- 특정 이름 해당 창
  - 존재 시
    - 해당 창 내 url 내용 로드
  - 부재 시
    - 새 창 표시
- name of the new window. Each window has a `window.name`, and here we can specify which window to use for the popup. If there's already a window with such name – the given URL opens in it, otherwise a new window is opened.
