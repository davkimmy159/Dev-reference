...
##### 생성자 별 `ArrayBuffer` <sub>(객체)</sub> 생성 여부
- 내부 자동 생성
  - ② ~ ⑤
- 미생성
  - ①
...
let num = 256;
alert(num.toString(2)); // 100000000 (이진수)
...
- `ArrayBufferView` <sub>(뷰 객체)</sub>
- `BufferSource` <sub>(이진 데이터 처리 객체)</sub>
  - `ArrayBuffer` <sub>(객체)</sub>
  - `ArrayBufferView` <sub>(뷰 객체)</sub>
