...
##### `Uint8Array`
- treats each byte in `ArrayBuffer` as a separate number, with possible values are from 0 to 255 (a byte is 8-bit, so it can hold only that much). Such value is called a "8-bit unsigned integer".
...
##### `Float64Array`
- treats every 8 bytes as a floating point number with possible values from 5.0x10<sup>-324</sup> to 1.8x10<sup>308</sup>.
