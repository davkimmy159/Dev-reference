...
#### 2. `"` <sub>(1번째 패턴 문자)</sub> 검색 완료 후

##### 나머지 패턴 <sub>(`.+"`)</sub> 검색 시작
- 나머지 문자열 대상
  - 4번째 위치 시작
...
##### 검색
- 나머지 모든 위치
  - 패턴 일치 <sub>(문자열 끝 도달)</sub>

![witch_greedy3](../../images/03/07/10/witch_greedy3.svg)

#### 4. `.` 패턴 문자 반복 <sub>(`.+`)</sub> 종료 후

##### 다음 ,and tries to find the next character of the pattern. It's the quote `"`. But there's a problem: the string has finished, there are no more characters!
