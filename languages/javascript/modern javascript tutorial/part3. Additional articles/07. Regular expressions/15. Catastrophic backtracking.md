...
##### `(\w+\s?)*` → `(?=(\w+))\1`
1. `?=` <sub>(전방 탐색)</sub>
    - 현재 위치 단어 유무 여부 체크 <sub>(== 검색)</sub>
       - == 현재 위치 단어 검색
2. `(?=…)` <sub>(전방 탐색)</sub> 내용 그룹화
    - `(?=(…))`
    - contents of parentheses with `?=...` isn't memorized by the engine, so wrap `\w+` into parentheses. Then the engine will memorize their contents
