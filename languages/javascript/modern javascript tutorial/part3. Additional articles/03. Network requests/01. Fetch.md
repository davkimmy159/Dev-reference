...
#####  프라미스 <sub>(`fetch` 메서드 반환값)</sub> 즉시 이행 상태 전환
- [`Response`](https://fetch.spec.whatwg.org/#response-class) <sub>(내장 클래스)</sub> 인스턴스 함께
...
- 프라미스 <sub>(`fetch` 메서드 반환값)</sub> 거부 상태 전환
...
##### `headers` <sub>(`fetch` 메서드 `options` 객체 인수)</sub>
- 헤더 정보 객체
- `fetch` <sub>(메서드)</sub> 요청 헤더 설정
```javascript
...
|`Blob` · `BufferSource`|이진 데이터|
|`URLSearchParams`<br /><sub>(최근 드물게 사용)</sub>|`x-www-form-urlencoded`|
...
##### `Blob` <sub>(객체)</sub> · `BufferSource` <sub>(가상 이진 처리 객체)</sub>
- `fetch` <sub>(메서드)</sub> 이진 데이터 전송
...
#### `fetch` <sub>(메서드)</sub> 옵션 <sub>(`options` 객체 인수)</sub>

##### `method`
- HTTP 메서드

##### `headers`
- 요청 헤더 객체 <sub>(제약 사항 有)</sub>
...
- 응답 도착 시
  ...
  - 즉시 응답 읽기 시작 <sub>(`json()` 메서드)</sub>

##### `Promise.all`
- 모든 `fetch` <sub>(메서드)</sub> 요청 응답 완료 대기
```javascript
await Promise.all(names.map(name => fetch(…)));
```

##### 각 `fetch` <sub>(메서드)</sub> 별 본문 메서드 <sub>(`json()`)</sub> 호출
- 타 `fetch` <sub>(메서드)</sub> 요청 응답 대기 X
- 하위 수준 프라미스 API
  - 여전히 유용
```javascript
let jobs = [];
