...
##### `Blob` <sub>(객체)</sub> · `BufferSource` <sub>(뷰 객체)</sub>
...
|`formData()`|`FormData` <sub>(객체)</sub>|
|`blob()`|`Blob` <sub>(객체)</sub>|이진 데이터 <sub>(타입 有)</sub>|
|`arrayBuffer()`|`ArrayBuffer` <sub>(객체)</sub>|이진 데이터 <sub>(저수준)</sub>|
