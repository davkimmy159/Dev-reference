...
##### MB <sub>(용량)</sub> X · 64kb 이하
- 대용량 전송 시 주기적인 패킷 전송 필요
  - `onunload` 요청 시 잔여 용량 ↓
- 크기 제한 공유 <sub>(모든 `keepalive` 설정 요청)</sub>
  - 병렬 요청 전송 가능
    - 모든 요청 본문 총 길이 제한
    - but the sum of their body lengths should not exceed 64kb.
  - In other words, we can perform multiple `keepalive` requests in parallel, but the sum of their body lengths should not exceed 64kb.
