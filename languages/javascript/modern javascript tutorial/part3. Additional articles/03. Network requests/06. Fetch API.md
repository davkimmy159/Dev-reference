...
- `onunload` 요청 시
  - 잔여 용량 ↓
- 모든 `keepalive` <sub>(옵션)</sub> 설정 요청
  - 크기 제한 공유
- together. In other words, we can perform multiple `keepalive` requests in parallel, but the sum of their body lengths should not exceed 64kb.
