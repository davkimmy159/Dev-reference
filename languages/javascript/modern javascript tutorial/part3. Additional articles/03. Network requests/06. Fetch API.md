...
|값|설명|
|---|---|
|`"same-origin"` <sub>(기본값)</sub>|크로스 오리진 요청 시 미포함|
|`"include"`|항상 포함|
...
||HTTP 캐시 내 응답 無<br />- 일반 HTTP 요청 생성|
|`"only-if-cached"`|HTTP 캐시 내 응답 사용 <sub>(최신 · 구식 무관)</sub>|
||HTTP 캐시 내 응답 無<br />- 에러 발생|
||`mode: "same-origin"` <sub>(옵션)</sub> 설정 시<br />- 정상 동작|
