...
- 오리진 <sub>(전체 url X)</sub>
  - `http://site.com`
  - `http://site.com/path` X

|값|설명|
|---|---|
|`"no-referrer-when-downgrade"` <sub>(기본값)</sub>|항상 전체 url<br />- HTTPS → HTTP 제외|
|`"no-referrer"`|`Referer` <sub>(헤더)</sub> 無|
|`"origin"`|오리진 <sub>(전체 url X)</sub><br />- ex\) `http://site.com` <sub>(`http://site.com/path` X)</sub>|
|`"origin-when-cross-origin"`|동일 오리진 요청<br />- 전체 url|
||크로스 오리진 요청<br />- 오리진|
|`"same-origin"`|동일 오리진 요청<br />- 전체 url|
||크로스 오리진 요청<br />- `Referer` <sub>(헤더)</sub> 無|
|`"strict-origin"`|오리진|
||HTTPS → HTTP 요청<br />- `Referer` <sub>(헤더)</sub> 無|
|`"strict-origin-when-cross-origin"`|동일 오리진 요청<br />- 전체 url|
||크로스 오리진 요청<br />- 오리진|
||HTTPS → HTTP 요청<br />- `Referer` <sub>(헤더)</sub> 無|
|`"unsafe-url"`|항상 전체 url<br />- HTTPS → HTTP 포함|
...
##### 크로스 오리진 요청 시 오리진만 전송 설정 <sub>(url 경로 X)</sub>
```javascript
