...
||`DELETE`||
||…||
|`"Content-Type"`<br /><sub>(`headers`)</sub>|`"text/plain;charset=UTF-8"`|보통 자동 설정<br /><sub>(요청 본문 따라 값 상이)</sub>|
...
||`"manual"`||
||`"error"`||
|`integrity`|`""` <sub>(기본값)</sub>|ex\) `"sha256-abcdef1234567890"`|
|`keepalive`|`false` <sub>(기본값)</sub>||
||`true`||
|`signal`|`undefined` <sub>(기본값)</sub>|`AbortController`<br /><sub>(요청 취소)</sub>
...
let promise = fetch(url, {
  ...
  referrer: "about:client", // "" (헤더 생략)to send no Referer header,
                            // 현재 오리진 url
  ...
  integrity: "", // a hash  ("sha256-abcdef1234567890")
  keepalive: false, // true
  signal: undefined, // AbortController (요청 취소)
  window: window // null
});
```
...
##### `fetch` sets HTTP `Referer` <sub>(헤더)</sub>
