...
##### `id:`
- `lastEventId` <sub>(프로퍼티)</sub> 값 업데이트
- `Last-Event-ID` <sub>(헤더)</sub> 값 <sub>(재연결 시)</sub>

##### `retry:`
- recommends a retry delay for reconnections in ms. There's no way to set it from JavaScript.

##### `event:`
- event name, must precede `data:`.
