...
##### `[base]`
- 기초 url
  - `url` <sub>(1번째 인수 · 일부 url 경로)</sub> 기준
```javascript
// 두 url 동일
...
// https://google.com/search?q=Rock&Roll
alert(url2); 
```
...
<img src="../../images/commons/icons/circle-exclamation-solid.svg" /> **`URL` <sub>(객체)</sub>  
vs 인코딩 내장 함수 <sub>(인코딩 차이)</sub>**
