...
1. 안전한 요청
2. 이외 요청 <sub>(안전한 요청 X)</sub>

#### 안전한 요청
- 구현 난이도 ↓ <sub>(이의 요청 대비)</sub>

##### 2가지 조건 충족
1. [안전한 메서드](https://fetch.spec.whatwg.org/#cors-safelisted-method) 사용
    - GET
    - POST
    - HEAD

2. [안전한 헤더](https://fetch.spec.whatwg.org/#cors-safelisted-request-header) 사용
    - `Accept`
    - `Accept-Language`
    - `Content-Language`
    - 특정 `Content-Type`
