...
##### 과거
- 오직 `GET` · `POST`
  - 이외 HTTP 메서드 X <sub>(현대 다수 웹서버)</sub>
- `GET` · `POST` 외 요청 수신 시
  - 접근 권한 확인 <sub>(非브라우저발 요청 판단)</sub>
사용해 요청을 보낼 수 있을거란 상상조차 할 수 없었습니다. 아직까지도 이런 메서드를 다룰 수 없는 웹서버도 꽤 있죠. 이런 서버들은 `GET`, `POST` 이 외의 메서드를 사용한 요청이 오면 '이건 브라우저가 보낸 요청이 아니야'라고 판단하고 접근 권한을 확인합니다.
