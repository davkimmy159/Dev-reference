...
|요청|근본|실무|
|---|----|---|
|안전|`<form>` · `<script>` <sub>(태그)</sub> 사용 가능<br /><sub>(과거 ~ 현재)</sub>|즉시 요청 전송<br /><sub>(`Origin` 헤더 함께)</sub>|
|非안전|브라우저 전송 X|본 요청 즉시 전송 X<br />- preflight 사전 요청 전송<br />- 접근 권한 확인|

##### 근본적 차이
- 안전한 요청
  - 과거부터 `<form>` · `<script>` <sub>(태그)</sub> 사용 가능
- 안전하지 않은 요청
  - 브라우저 전송 X

##### 실무 관점
- 안전한 요청
  - `Origin` <sub>(헤더)</sub> 함께 즉시 요청
- 안전하지 않은 요청 전송
  - 본 요청 즉시 전송 X
  - preflight 사전 요청 전송
    - 접근 권한 확인
