...
##### `fetch` <sub>(메서드)</sub> 요청 <sub>(자격 증명 정보)</sub> 전송 설정
- `credentials: "include"`
```javascript
...
##### 서버측 자격 증명 정보 포함 요청 수신 동의 시
- 서버에서 받아들이기로 동의했다면 서버는 응답에 `Access-Control-Allow-Origin` 헤더와 함께 `Access-Control-Allow-Credentials: true` 헤더를 추가해서 보냅니다.
