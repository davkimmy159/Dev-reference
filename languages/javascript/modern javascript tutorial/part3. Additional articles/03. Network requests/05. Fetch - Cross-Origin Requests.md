...
##### `<iframe>` · 페이지 양쪽 특수 스크립트 작성
- 제약 <sub>(`<iframe>` 콘텐츠 읽기 X)</sub> 우회
  - 타 사이트 양방향 통신 가능
이런 제약 역시 피할 수 있는 트릭을 만들었습니다. 이렇게 iframe을 사용한 트릭은 오리진이 다른 사이트간에도 양방향 통신이 가능하도록 해주었죠. 어떤 스크립트를 사용하면 가능한지에 대해선 다뤄도 의미가 없기 때문에 상세한 내용은 넘어가도록 하겠습니다.
