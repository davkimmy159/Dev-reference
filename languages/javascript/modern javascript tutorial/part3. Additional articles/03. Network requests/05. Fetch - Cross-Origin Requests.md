...
||안전|非안전|
|---|----|---|
|근본 <sub>(이론)</sub>|과거부터 `<form>` <sub>(요소)</sub> · `<script>` <sub>(태그)</sub> 사용 가능|과거 브라우저 전송 불가능|
|실무|즉시 전송 <sub>(`Origin` 헤더 포함)</sub>|즉시 전송 X<br />-preflight' <sub>(사전 요청)</sub> 전송<br />&nbsp;&nbsp;- 접근 권한 확인|
