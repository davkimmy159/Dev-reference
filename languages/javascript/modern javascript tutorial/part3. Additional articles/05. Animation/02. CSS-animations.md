...
transition-delay엔 음수 값도 넣을 수 있습니다. 값이 음수일 땐 애니메이션 효과가 중간부터 나타납니다. transition-duration을 2s, 지연 시간을 -1s로 설정하면 애니메이션 효과는 1초가 지난 후 1초 동안 지속됩니다.
...
1. 첫 번째 조절점: (0,0)
2. 마지막 조절점: (1,1)
3. 중간 조절점들: x가 0과 1 사이에 있어야 함. y엔 제약이 없음
...
- 그리고 나서 left가 400px일 때의 위치로 후진합니다.
...
아시다시피, timing 함수의 y축은 '애니메이션 프로세스의 완성도’를 나타냅니다. y가 0일 땐 프로퍼티 시작 값을, y가 1일 땐 프로세스 종료 값을 의미하죠. 그렇기 때문에 y가 0보다 작아지면 left 프로퍼티가 시작 값인 100px 보다 낮게 설정되고, y가 1보다 커지면 left 프로퍼티가 끝값인 400px보다 커지게 됩니다.

이 예시에선 y 값이 정상 범위를 크게 벗어나지 않기 때문에 ‘부드럽게’ 변형이 일어납니다. 그런데 y 값이 -99나 99가 되면 기차가 앞·뒤로 아주 크게 움직이겠죠?
...
- step-end – steps(1, end)와 같습니다. transition-duration의 끝에 애니메이션이 한 단계만 진행됩니다.
...
@keyframes을 다루는 글이나 [명세서](https://drafts.csswg.org/css-animations/)를 읽으면 더 많은 정보를 찾을 수 있으니 참고해보시기 바랍니다.
