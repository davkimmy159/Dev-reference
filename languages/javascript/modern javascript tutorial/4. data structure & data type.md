...
- 가장 큰 인덱스 + 1 (배열 내 요소 개수 X)
  - 배열 요소 하나, 이 요소 인덱스 아주 큰 정수 시 length 프로퍼티 아주 커짐
  ```javascript
  // 잘못된 배열 사용
  let fruits = [];
  fruits[123] = "사과";
...
- 요소 검색 시 완전 항등 연산자 '===' 사용
  - false 검색 시면 정확히 false만 검색, 0 검색 X
- includes
...
- 인수 함수
  ...
  - 적용 결과 : 다음 함수 호출 시 사용
- 함수 인수
  - initial 　　　: 함수 최초 호출 시 사용 초깃값 (옵션)
  - accumulator : 이전 함수 호출 결과
  - item 　　　&nbsp;&nbsp;: 현재 배열 요소
  - index 　　　&nbsp;: 요소의 위치
  - array 　　　&nbsp;&nbsp;: 배열
- 이전 함수 호출 결과
...
- range 이터러블 만들려면 (for..of 동작하도록 하려면)
  - 객체에 Symbol.iterator (특수 내장 심볼) 메서드 추가
    ...
    4. next() 반환 값
        - 형태 　　　: { done: Boolean, value: any }
        - done=true &nbsp;: 반복 종료 의미
        - done=false : value 다음 값 저장
- 직접 이터러 객체 생성해보기
...
- 주요 메서드
  ...
  - set.delete(value)
    - 값 제거
      - 셋 내 값 있어 제거 성공 시 : true 반환
      - 셋 내 값 없어 제거 실패 시 : false 반환
  - set.has(value)
    - 셋 내 값 존재 시 : true 반환
    - 셋 내 값 부재 시 : false 반환
  - set.clear()
