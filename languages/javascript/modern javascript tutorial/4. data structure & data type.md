...
|---|---|
|`\n`|줄 바꿈|
|`\r`|캐리지 리턴 (carriage return)<br />Windows 에서 캐리지 리턴, 줄 바꿈 특수 문자 조합해 줄 바꿈<br />- `\r\n`<br />캐리지 리턴 단독 사용 경우 X|
|`\'` · `\"`|따옴표|
|`\\` |역슬래시|
|`\t`|탭|
...
arr.slice([start], [end])
```
- "`start`" ~ "`end`" 인덱스 ("`end`" 제외) 요소 복사한 새로운 배열 반환
- "`start`" · "`end`" 음수
  - 배열 끝부터 요소 개수
- 인수 X
  - 복사본 생성
...
### 배열 메서드와 `thisArg`
- 함수 호출 대부분의 배열 메서드 (`find` · `filter` · `map` 등, `sort` 제외)
  - `thisArg` 매개변수 옵션
  - 가끔 사용되는 인수
```javascript
...
- 배열 외 다수 내장 객체 반복 가능
  - 문자열 역시 이터러블
- 배열 아닌 객체 어떤 것들의 컬렉션 (목록, 집합 등) 나타낼 경우 `for…of` 문법 적용 시 컬렉션 순회 유용

### `Symbol.iterator`
- `range` 이터러블 만들려면 (`for…of` 동작하도록 하려면)
  - 객체에 `Symbol.iterator` (특수 내장 심볼) 메서드 추가
    1. `for…of` 시작 시 `for…of` 가 `Symbol.iterator` 호출
        - `Symbol.iterator` 없으면 에러 발생
        - `Symbol.iterator` 반드시 이터레이터 (iterator, 메서드 `next` 보유 객체) 반환
    2. 이후 `for…of` 반환된 객체 (이터레이터) 만 대상으로 동작
...
- 단점
  - 두 개 `for…of` 반복문 하나의 객체 동시 사용 X
    - 이터레이터 (객체 자신) 하나뿐 → 두 반복문 반복 상태 공유
    - 동시 두 개 `for…of` 사용 → 비동기 처리에서도 흔한 케이스 X

#### 무한개의 이터레이터
- 무수히 많은 이터레이터 가능
- `range.to` 에 `Infinity` 할당 시 `range` 무한대
  - 무수히 많은 의사 난수 (pseudorandom numbers) 생성 이터러블 객체 가능
    - 이 방법 유용한 경우 有
- `next` 제약사항 X
  - `next` 값 계속 반환
    - 정상적인 동작
...
### 맵의 요소에 반복 작업하기
- `map.keys()`
  - 각 요소 키 모은 반복 가능한 (iterable, 이터러블) 객체 반환
- `map.values()`
...
- 주요 메서드
  - `new Set(iterable)`
    - 셋 생성
    - 이터러블 객체 전달받으면 (대개 배열 전달받음) 그 안 값 복사해 셋 삽입
  - `set.add(value)`
    - 값 추가 후 셋 자신 반환
  - `set.delete(value)`
    - 값 제거
...
- 자바스크립트 엔진 도달 가능한 (추후 사용 가능성 있는) 값 메모리에 유지
```javascript
let john = { name: "John" };
  ...
  - 가비지 컬렉션 동작 방식
    ...
    - 가비지 컬렉션 동작 시점 정확히 인지 X
      ...
      - 한 번에 메모리 청소할 수 or 부분 부분 메모리 청소할 수도
        ...
        - 위크맵 요소 (키 · 값) 전체 대상 무언가를 동작 자체 불가능
...
- 특정 사용자 객체 메모리에서 사라지면 해당 객체 대한 정보 (방문 횟수) 손수 지워줘야 하는 상황
...
- 위크맵 키 (`"john"`) 대응 값 (`"john"` 방문 횟수) 자동 가비지 컬렉션 대상
...
```
- 위크맵 문제 예방
- 객체 메모리에서 삭제 시, 캐시 저장 결과 (함수 연산 결과) 역시 메모리에서 자동 삭제
```javascript
// 📁 cache.js
...
- 포괄적인 용도
  - 메서드 적용 자료구조 일련 합의 준수
    - 커스텀 자료구조 대상 순회 시
      - 기존 메서드 사용 X
      - 직접 메서드 구현
- 일반 객체용 순회 관련 메서드 (자료구조 메서드 문법 상이)
  - `Object.keys(obj)`
    - 객체 키 배열 반환
  - `Object.values(obj)`
    - 객체 값 배열 반환
...
### 객체 변환하기
- 객체 대상 배열 전용 메서드 (`map` · `filter` 등) 사용 X
...
- 모든 이터러블 (iterable, 반복 가능 객체) 적용 가능
```javascript
let [a, b, c] = "abc"; // ["a", "b", "c"]
...
- 할당 연산자 좌측
  - ‘할당할 수 있는(assignables)’ 것이라면 어떤 것이든 가능
  - 객체 프로퍼티 가능
```javascript
let user = {};
...
```
- `=`
  - 할당 값 없을 때 '기본값 (default value)' 설정
```javascript
// 기본값
...
- UTC 기준(UTC+0) 1970.1.1 00:00:00 에서 milliseconds 밀리초 (1/1000 초) 후 시점 저장된 `Date` 객체 반환
```javascript
...
- 주어진 인수 조합해 생성 가능 날짜 저장된 객체 반환 (지역 시간대 기준)
- 필수 : 1 · 2번째 인수
  - `year`
    - 4자리 숫자
      - 2013 &nbsp;O
...
- 여러 자바스크립트 엔진 더는 사용되지 않는 (`deprecated`) 비표준 메서드 `getYear()` 구현
- 두 자릿수 연도 반환 경우 有
  - 절대 사용 X
  - `getFullYear()` 사용
...
#### `getTime()`
- 주어진 일시 ~ 1970.1.1 00:00:00 사이 간격 (밀리초 단위) 타임스탬프 반환
...
- 날짜 구성요소 설정 메서드
  ...
  - `setSeconds(sec, [ms])`
  - `setMilliseconds(ms)`
  - `setTime(milliseconds)` (1970.1.1 00:00:00 UTC ~ 밀리초 이후 나타내는 날짜 설정)
...
- `Date` 객체 숫자형 변경 시 타임스탬프 (`date.getTime()` 호출 시 반환 값)
```javascript
let date = new Date();
...
### 벤치마크 테스트
- 비교 대상 성능 비교 · 시험 · 평가
- CPU 소모 큰 함수 신뢰할만한 벤치마크 (평가 기준) 구하려면 상당한 주의 필요
```javascript
...
- 모던 자바스크립트 엔진 아주 많이 실행된 코드인 'hot code’ 대상 최적화 수행
  - 메인 반복문 실행 전 예열용 (heat-up)  bench 실행
```javascript
// 메인 반복문 실행 전, "예열용" 코드
bench(diffSubtract);
bench(diffGetTime);
...
- 함수 최초 호출 때 `{"": meetup}` 형태 "래퍼 객체" 생성
  - 문자열 `":[object Object]"`
  - `replacer` 함수 가장 처음 처리하는 (key, value) 쌍
    - 키 : 빈 문자열
    - 값 : 변환 대상 객체 (`meetup`) 전체
...
- `str`
  - JSON 형식 문자열
- `reviver`
  - 모든 (key · value) 쌍 대상 호출 `function(key,value)` 함수
    - 값 변경
```javascript
// 문자열 변환 배열
let numbers = "[0, 1, 2, 3]";
